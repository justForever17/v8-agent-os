...
test("admin sidebar exposes task groups with discoverable primary and secondary entries", () => {
    const source = fs.readFileSync(path.join(adminRoot, "src", "admin", "components", "layout", "Sidebar.tsx"), "utf8");
...
test("admin sidebar group labels are localized in Chinese", () => {
    const locale = JSON.parse(fs.readFileSync(path.join(adminRoot, "src", "admin", "i18n", "locales", "zh-CN.json"), "utf8"));
...
test("admin sign-out clears the session without accepting a cross-origin callback", () => {
    const sidebar = fs.readFileSync(path.join(adminRoot, "src", "admin", "components", "layout", "Sidebar.tsx"), "utf8");
    ...
    assert.match(sidebar, /if \(remainingSession\?\.user\?\.adminAuthenticated === true\) throw new Error\("admin_session_not_cleared"\)/);
