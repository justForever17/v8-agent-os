...
function modelHubSource() {
  return readText("src/app/admin/(dashboard)/model-hub/page.tsx")
    + "\n" + readText("src/admin/lib/model-hub/catalog.ts")
    + "\n" + readText("src/admin/lib/model-hub/channels.ts")
    + "\n" + readText("src/admin/lib/model-hub/audio.ts")
    + "\n" + readText("src/admin/lib/model-hub/models.ts")
    + "\n" + readText("src/admin/lib/model-hub/editor-drafts.ts")
    + "\n" + readText("src/admin/components/model-hub/ModelEditorDialog.tsx")
    + "\n" + readText("src/admin/components/model-hub/ProviderEditorDialog.tsx")
    + "\n" + readText("src/admin/hooks/use-model-hub-bootstrap.ts");
}

test("Model Hub exposes the provider model as the title and keeps the complete request route as secondary evidence", () => {
  const providerCard = readText("src/admin/components/models/ProviderCard.tsx");
  const modelCard = readText("src/admin/components/models/ModelCardV2.tsx");
  const modelAssets = readText("src/admin/lib/models/model-assets.ts");
...
test("Admin model projection preserves media route evidence for the visible model card", () => {
  const projection = readText("src/admin/lib/models/model-admin.ts");
  const zh = JSON.parse(readText("src/admin/i18n/locales/zh-CN.json"));
  const en = JSON.parse(readText("src/admin/i18n/locales/en.json"));
...
test("Manual and quick model setup share the canonical binding write surface", () => {
  const manualCreate = readText("src/app/api/admin/models/route.ts");
  const manualUpdate = readText("src/app/api/admin/models/[id]/route.ts");
  const quickConnect = readText("src/app/api/admin/models/connect/route.ts");
