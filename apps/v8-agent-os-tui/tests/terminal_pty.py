...
def scenario(reader=False):
    ...
    editor_script = root / 'editor.py'
    editor_audit = root / ('editor-reader.json' if reader else 'editor-repaint.json')
    editor_script.write_text("import json, pathlib, sys, termios\n"
        "source=pathlib.Path(sys.argv[-1])\n"
        "flags=termios.tcgetattr(0)[3]\n"
        "pathlib.Path(sys.argv[1]).write_text(json.dumps({'canonical':bool(flags & termios.ICANON),'echo':bool(flags & termios.ECHO),'before':source.read_text()}))\n"
        "source.write_text(source.read_text()+'\\n外部编辑👩‍🚀')\n")
    env = {**os.environ, 'TERM': 'xterm-256color', 'NO_COLOR': '1', 'V8_AGENT_OS_HOME': str(root), 'VISUAL': f'python3 "{editor_script}" "{editor_audit}"'}
    ...
    try:
        ...
        # Navigate visual columns through a short line, preserving a ZWJ glyph.
        send('\x03')
        vertical = 'ab中👨‍👩‍👧‍👦Z\nx\n12👍🏽45'
        send('\x1b[200~' + vertical + '\x1b[201~')
        send('\x1b[A\x1b[A'); send('!', .6)
        saved = json.loads((root / 'runtime/tui/view.json').read_text())
        edited = saved['drafts'][saved.get('sessionId') or 'new']['text']
        assert edited == 'ab中👨‍👩‍👧‍👦!Z\nx\n12👍🏽45', repr(edited)
        # Visible search and exact matching work in repaint and linear modes.
        send('\x10'); send('editor')
        assert '搜索：editor' in output.decode(errors='replace')
        assert '匹配 1 /' in output.decode(errors='replace')
        send('\r', 1.5)
        audit = json.loads(editor_audit.read_text())
        assert audit['canonical'] and audit['echo'], audit
        assert audit['before'] == edited
        saved = json.loads((root / 'runtime/tui/view.json').read_text())
        draft = saved['drafts'][saved.get('sessionId') or 'new']
        assert draft['text'] == edited + '\n外部编辑👩‍🚀'
        assert 'unknown' not in draft
        assert not termios.tcgetattr(slave)[3] & termios.ICANON, 'raw input not restored after editor'
        send('\r'); assert child.poll() is None
        saved = json.loads((root / 'runtime/tui/view.json').read_text())
        assert 'unknown' not in saved['drafts'][saved.get('sessionId') or 'new'], 'editor returned with automatic submit enabled'
