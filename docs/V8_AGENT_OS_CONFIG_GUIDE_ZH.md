...
内置子代理的专业指导位于 `~/.v8-agent-os/agents/<角色 ID>.md`，可在 Admin 子代理页或本地编辑。新建默认文件仅包含该角色的通用专业能力与工作方法；工具、权限、任务及运行约定由 Engine 在执行时注入，编辑或清空正文不会扩大权限。已有自定义 frontmatter 仍可读取，模型绑定继续由 Model Hub 管理。

升级只自动替换能与已发布种子完整内容校验匹配的文件，并先备份到 `backups/agents/`。正文、配置或注释有改动，以及无法确认来源的旧文件都会保留。要恢复某个角色的默认指导，可先将对应 Markdown 备份并移出 `agents` 目录，再重启 Engine 重新生成；这不需要删除 Model Hub 中的角色模型绑定。角色修改会用于后续新任务，已开始执行的任务保留它所绑定的上下文。

Research 内部检索、写作与审核阶段使用独立的内置阶段合同，只复用注册角色的模型配置；该内部流程不读取可编辑角色正文。普通命名子代理任务会加载其专业正文。
...
在 Model Hub 或供应商详情页编辑模型：

- **上下文使用预算**是用户愿意提供给该模型的上下文额度。支持 1M 的模型可以设为 500000 或更低的正整数；小于 256K 只产生建议，不阻止保存和角色绑定。较小预算会更早触发上下文压缩，但不改变供应商真实能力。
- **输出预算：自动**不使用预置估算值限制请求。参数可省略的接口交给供应商默认值；Anthropic 等必须提供该参数的接口使用应用请求预算，当前为 32768，不将它登记为模型能力。自动不是无限输出，也不保证供应商不会截断。
- **输出预算：自定义上限**保留用户设定，实际请求不能超过它；有明确较小单次预算的任务仍可使用更小值。思考 token 可能占用同一输出预算。若显式思考预算与输出上限冲突，应调整配置，而不是静默抬高上限。

升级时只自动迁移已确认的旧估算来源：`v8_conservative_2026_default`、`estimated` 且值仍为 4096。人工设置、来源不明及已改过的值保留；不能仅凭“等于 4096”判断。旧数值保留供回退，但自动模式下不再作为请求上限。预置目录是接入默认值，重新接入也不得覆盖已保存的使用预算。

供应商拒绝、上下文溢出或输出截断仍须如实报告。以上设置不能突破供应商的真实 API 限制，也不替代运行次数、时间和费用治理。
...
Phone 的「连接」页提供独立「配置分发」面板。选择模板与已信任从设备，查看每台设备的差异和目标映射，再明确确认本批已准备的目标。主从角色可在面板设置；两端都须认可各自的主从角色，单端设置不会远程修改另一端授权。

- **模型预算与参数**：分发全局/单次/默认项目数值预算和内置模型角色温度，不包含具体项目预算或路径。
- **模型角色映射**：为源设备的内置角色选择目标设备已有的兼容模型。目标缺少本地凭据或明确认证合同，会列出缺项并阻止该目标提交。

Owner、密钥、长期凭据、grant、provider 记录、工作区及绝对路径不参与分发。每个目标仍通过本地 Config Broker 的准备、提交与读回验证；不复制整个 `config.json`。

逐台回执保留部分成功：一台离线不妨碍其他目标提交。已确认的离线目标可在 Engine 重启或重连后继续原计划；初次离线、冲突或重新准备的目标须在展示新差异后重新确认。相同命令不会二次提交。

「取消未应用目标」保留已经生效的配置；「撤回已应用配置」按原事务精确恢复。若目标在提交后另有修改，撤回会报告冲突，不覆盖更新。撤销信任后不能从源设备强行回滚，应恢复合法连接或在目标本地处理。原 Phone 被撤销后，当前有效 Owner 仍可取消/撤回旧作业。

回执未知时重新准备会先核对原事务，不能遗忘已提交结果。模型映射缺失的目标会单独列出，就绪目标仍可预览并确认；之后可修改未确认目标映射。补好本地凭据后可直接刷新目标能力。作业列表优先展示未完成作业并分页，点开才读取完整回执；逐台详情按需展开。

「当前设备的本地目录映射」只操作当前 Phone 连接的 Engine：选择该设备已有项目、核对目录并明确信任，再绑定到指定 peer 连接。若要设置另一台设备，先切换其已配对 profile；本机目录不作为远程配置模板发送。首次添加项目仍复用聊天中的工作区选择与信任流程。
