...
test("computer use availability only waits on an explicit environment refresh", () => {
  const proxy = read("src/app/api/admin/computer-use/availability/route.ts");
  ...
  assert.match(page, /availabilityRefreshKey > 1[\s\S]*\/api\/admin\/computer-use\/availability\?refresh=true/);
  ...
  assert.match(page, /requestUrl = attempt > 0 \? "\/api\/admin\/computer-use\/availability" : availabilityUrl/);
