...
function mount() {
  ...
  const read = url => structuredClone(url.includes('/catalog') ? fixture.catalog : url.includes('/health') ? fixture.health : url === '/api/admin/models' ? [] : { items: [] });
  const imports = name => {
...
function inlineSaveStateText(ui) {
  const sourcePath = path.resolve(__dirname, '../../src/admin/components/admin-shell/InlineSaveState.tsx');
  const source = ts.transpileModule(fs.readFileSync(sourcePath, 'utf8'), {
    fileName: sourcePath,
    compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022, jsx: ts.JsxEmit.ReactJSX },
  }).outputText;
  const locale = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../../src/admin/i18n/locales/zh-CN.json'), 'utf8'));
...
for (const kind of ['form', 'json']) {
  ...
  test(`${kind} MCP editor preserves denied drafts and retries through the same endpoint`, async () => {
    ...
    assert.equal(ui.requests[0].url, '/api/admin/mcp/config');
