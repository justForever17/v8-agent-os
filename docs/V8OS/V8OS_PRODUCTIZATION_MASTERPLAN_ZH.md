...
- 仍是源码树预览版，不是正式安装器。
- 没有自动更新、代码签名、桌面 GitHub release workflow。
...
当前已有 GitHub Actions：`phone-v*` tag 可触发 APK artifact / release。
...
- 建立桌面 release workflow 之前，不能宣称桌面版正式可安装。
- Phone tag release 继续保持可用。
