...
import { type CatalogProvider, type CatalogPurpose, type CatalogRuntimeProtocol, type CatalogModel, type CustomProviderCapability, buildCatalogProvidersForPurpose, getCatalogPurposeConfig, catalogProviderChannels, isXiaomiAnthropicBaseUrl, getModelTypeForPurpose, CATALOG_PURPOSES, CUSTOM_PROVIDER_CAPABILITIES, previewModelsUrl } from "@admin/lib/model-hub/catalog";
import { resolveProviderLogo, resolveModelIcon } from "@admin/lib/models/model-assets";
import { useModelHubBootstrap } from "@admin/hooks/use-model-hub-bootstrap";
import { useState, useRef, useMemo, useEffect } from "react";
import { useToast } from "@admin/components/ui/use-toast";
import { useT } from "@admin/components/providers/LocaleProvider";
import { type AIProvider, type AIModel, type ModelConnectionStatus, type ModelReasoningRepairStatus } from "@admin/lib/model-hub/types";
import { type ProviderChannel } from "@admin/lib/models/provider-admin";
import { createProviderChannel, PROVIDER_CHANNEL_PRESETS } from "@admin/lib/model-hub/channels";
import { type AudioVoiceOption, type TtsVoiceProviderInfo, type TtsVoiceDesignCandidate, hasAudioInputCapability, hasAudioOutputCapability, localizeAudioVoicePresets, resolveAudioVoicePresetKey, voicePresetsForCustomTtsProtocol, voiceManagerErrorText, mergeAudioConfig, sttEndpointPlaceholder, headerValueForInput, ttsEndpointPlaceholder } from "@admin/lib/model-hub/audio";
import { modelRefFor, modelMatchesTab, modelLabel } from "@admin/lib/model-hub/models";
import { readJsonErrorMessage, extractErrorText } from "@admin/lib/model-hub/errors";
...
import { SearchableVoiceSelect } from "@admin/components/models/SearchableVoiceSelect";
...
import { HydrationSafeClientOnly } from "@admin/components/ui/hydration-safe-client-only";
import { Checkbox } from "@admin/components/ui/checkbox";
import { EmptyState } from "@admin/components/admin-shell/EmptyState";
import { ProviderCard } from "@admin/components/models/ProviderCard";
import { Tabs, TabsList, TabsTrigger } from "@admin/components/ui/tabs";
import { ModelCardV2 } from "@admin/components/models/ModelCardV2";
import { SourceMetaRow } from "@admin/components/admin-shell/SourceMetaRow";
import { ProviderEditorDialog } from "@admin/components/model-hub/ProviderEditorDialog";
import { ModelEditorDialog } from "@admin/components/model-hub/ModelEditorDialog";
...
export default function ModelHubPage() {
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                const capabilitiesResponse = await fetch("/api/admin/audio/model-ref-voices", {
                    method: "POST",
                ...
                const listResponse = await fetch("/api/admin/audio/model-ref-voices", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
    ...
    const handleSetReasoningLevel = async (model: AIModel, controlMeta: ControlPlaneModel | null, level: string) => {
        ...
        const response = await fetch(`/api/admin/models/${encodeURIComponent(model.id)}?providerId=${encodeURIComponent(model.providerId)}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
    ...
    const handleDeleteModel = async (model: {
        ...
        try {
            const response = await fetch(`/api/admin/models/${encodeURIComponent(model.id)}?providerId=${encodeURIComponent(model.providerId)}`, { method: "DELETE" });
            if (!response.ok) {
    ...
    const handleProbeCatalogProvider = async () => {
        ...
        setManualModelEntryEnabled(false);
        try {
            const response = await fetch("/api/admin/models/providers/probe", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    const handleConnectCatalogModel = async (providerId: string, modelId: string, apiKey = "") => {
        ...
        try {
            ...
            const isMediaPurpose = catalogPurpose !== "chat";
            const customAnthropicRuntime = isCustomProvider && catalogPurpose === "chat" && isXiaomiAnthropicBaseUrl(baseUrl);
            const response = await fetch("/api/admin/models/connect", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    providerId,
    ...
    const handleDeleteCustomCatalogProvider = async (providerId: string) => {
        ...
        const response = await fetch(`/api/admin/models/providers/custom/${encodeURIComponent(providerId)}`, { method: "DELETE" });
        const data = await response.json().catch(() => ({}));
        if (!response.ok || data.ok === false) {
            toast({
                variant: "destructive",
    ...
    const handleFetchCustomTtsVoices = async () => {
        ...
        try {
            const response = await fetch("/api/admin/audio/voices", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    const handleFetchModelRefTtsVoices = async (notify = true): Promise<AudioVoiceOption[] | null> => {
        ...
        setModelRefTtsVoiceListState("loading");
        try {
            const response = await fetch("/api/admin/audio/model-ref-voices", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ action: "list", modelRef: selectedTtsModelRef }),
            });
    ...
    const handlePreviewTts = async () => {
        ...
        try {
            const response = await fetch("/api/admin/audio/tts/preview", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    const handleTestConnection = async (modelRef: string) => {
        ...
        try {
            ...
            const response = await fetch("/api/admin/models/test-connection", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    const handleRepairReasoning = async (modelRef: string) => {
        ...
        try {
            const target = models.find((item) => (item.modelRef || item.id) === modelRef);
            const response = await fetch("/api/admin/models/repair-reasoning", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ modelRef, modelId: target?.modelId, providerId: target?.providerId }),
            });
