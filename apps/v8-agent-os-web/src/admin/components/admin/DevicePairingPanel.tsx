...
const DEVICE_SESSIONS_URL = "/api/admin/client/devices";
...
export function DevicePairingPanel({ onConfigure }: { onConfigure?: () => void } = {}) {
    ...
    const loadAddress = useCallback(async (signal?: AbortSignal) => {
        ...
        try {
            const response = await fetch("/api/admin/client/link/manifest", { cache: "no-store", signal });
    ...
    async function createTicket() {
        ...
        setCopied(false);
        try {
            const response = await fetch("/api/admin/client/pairing/tickets", {
                method: "POST",
    ...
    async function revokeDevice(deviceSessionId: string) {
        ...
        setError("");
        try {
            const response = await fetch("/api/admin/client/devices", {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
