...
import { OutputTokenBudgetField } from "@admin/components/models/OutputTokenBudgetField";
import type { ControlPlaneModel, ControlPlanePayload, ProviderOverview } from "@admin/components/models/control-plane-types";
...
import { getPlatformLoginPresetConfig, inferPlatformLoginPreset, PLATFORM_LOGIN_PRESETS, type PlatformLoginPreset } from "@admin/lib/models/provider-admin";
...
export default function ProviderConfigPage({ params
  ...
  const { id } = use(params);
  const providerPath = `/api/admin/providers/${id}`;
  ...
  const [controlPlane, setControlPlane] = useState<ControlPlanePayload | null>(() => peekAdminJsonCache<ControlPlanePayload>("/api/admin/model-control-plane") || null);
  const [defaultModelId, setDefaultModelId] = useState<string | null>(() => {
    const cached = peekAdminJsonCache<{ modelRef?: string; modelId?: string }>("/api/admin/settings/default-agent-model");
  ...
  const fetchProvider = useCallback(async (force = false) => {
    ...
    try {
      const [nextProvider, nextControlPlane, defaultModel] = await Promise.all([
        ...
        fetchAdminJson<ControlPlanePayload>("/api/admin/model-control-plane", { force }),
        fetchAdminJson<{ modelRef?: string; modelId?: string }>("/api/admin/settings/default-agent-model", { force })
      ]);
      if (nextProvider) {
  ...
  const handleSaveModel = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    `/api/admin/models/${encodeURIComponent(editingModel.id)}?providerId=${encodeURIComponent(editingModel.providerId)}` :
    "/api/admin/models";
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                t("app.admin.dashboard.models.providers.id.page.k560df989")} readOnly />
                            ...
                            {platformProviderSelected || credentialMode === "oauthFile" ? <div className="grid gap-2">
                                    ...
                                    <div className="flex items-center overflow-hidden rounded-xl border border-input bg-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-0">
                                        <span className="shrink-0 border-r border-border/60 px-3 text-sm text-muted-foreground">oauth:</span>
                                        <Input id="oauthPath" name="oauthPath" className="rounded-none border-0 shadow-none focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-offset-0" value={providerOauthPath} onChange={(event) => setProviderOauthPath(event.target.value)} placeholder={activePlatformPreset.oauthPath} />
                                    </div>
