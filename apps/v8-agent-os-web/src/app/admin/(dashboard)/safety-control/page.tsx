...
import { SafetyGuardianPanel } from "@admin/components/runtime/SafetyGuardianPanel";
...
function normalizeSafetyData(input: SafetyData): SafetyData {
  return {
    ...
    activeDefense: {
      ...
      knownListeningPorts: Array.isArray(input.activeDefense?.knownListeningPorts) ? input.activeDefense?.knownListeningPorts : ["tcp:9527", "tcp:9530"]
    }
  };
}
...
export default function SafetyControlPage() {
  ...
  const [initialState] = useState(() => {
    ...
    return {
      ...
      dashboard: peekAdminJsonCache<SafetyDashboard>("/api/admin/safety/dashboard?limit=80") ?? null,
  ...
  const loadConfig = useCallback(async (force = false) => {
    ...
    try {
      const [configResult, modelsResult, dashboardResult] = await Promise.allSettled([
        ...
        fetchAdminJson<SafetyDashboard>("/api/admin/safety/dashboard?limit=80", { force, ttlMs: 10_000 }),
      ]);
  ...
  const handleApprovalAction = async (approvalId: string, approve: boolean) => {
    ...
    try {
      const response = await fetch(`/api/admin/approvals/${encodeURIComponent(approvalId)}/${approve ? "approve" : "reject"}`, {
        method: "POST",
        headers: {
  ...
  const handleSkillSafetyAction = async (reviewId: string, action: "approve" | "disable" | "revoke" | "rescan") => {
    ...
    setGovernanceBusy(busyKey);
    try {
      const response = await fetch(`/api/admin/skills/safety/reviews/${encodeURIComponent(reviewId)}/${action}`, {
        method: "POST"
      });
      if (!response.ok) {
  ...
  const handleAllowlistRevoke = async (entryId: string) => {
    ...
    setGovernanceBusy(busyKey);
    try {
      const response = await fetch(`/api/admin/safety/allowlist/${encodeURIComponent(entryId)}/revoke`, {
        method: "POST"
      });
      if (!response.ok) {
  ...
  const handleActiveDefenseIncidentAction = async (incidentId: string, action: "confirm" | "ignore") => {
    ...
    setGovernanceBusy(busyKey);
    try {
      const response = await fetch(`/api/admin/safety/active-defense/incidents/${encodeURIComponent(incidentId)}/${action}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
