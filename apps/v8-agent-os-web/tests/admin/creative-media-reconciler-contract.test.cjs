...
test("creative media bootstrap fetches the reconciler governance projection", () => {
  const source = readAdmin("src", "app", "api", "admin", "creative-media", "bootstrap", "route.ts");
...
test("creative media admin global reads use the explicit governance surface", () => {
  const proxy = readAdmin("src", "app", "api", "admin", "creative-media", "[...path]", "route.ts");
  const page = readAdmin("src", "app", "admin", "(dashboard)", "creative-media", "page.tsx");
  const runtimeConfig = readAdmin("src", "admin", "lib", "server", "runtime-config.ts");
  ...
  assert.match(page, /\/api\/admin\/creative-media\/governance\/snapshot/);
  assert.match(page, /\/api\/admin\/creative-media\/governance\/work-orders/);
