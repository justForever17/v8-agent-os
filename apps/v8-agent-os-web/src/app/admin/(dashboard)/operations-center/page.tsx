...
import AuditLogsPanel from "@admin/components/memory/AuditLogsPanel";
import { PendingApprovalsPanel } from "@admin/components/runtime/PendingApprovalsPanel";
import { RecentRunsPanel } from "@admin/components/runtime/RecentRunsPanel";
import { useRuntimeOpsData } from "@admin/components/runtime/use-runtime-ops";
import { formatRuntimeKindLabel } from "@admin/components/runtime/use-runtime-ops";
...
import { tg, ti } from "@admin/i18n/admin-legacy";
...
function StorageRetentionPanel() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/admin/storage-retention/stats", {
        cache: "no-store"
      });
  ...
  const run = async (kind: "dry-run" | "prune" | "compact") => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/storage-retention/${kind}`, {
        method: "POST",
        headers: {
  ...
  const refreshRegistry = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/admin/storage-retention/registry/refresh", {
        method: "POST",
        cache: "no-store",
      });
      const payload = await response.json().catch(() => null);
  ...
  const saveBudgets = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/admin/storage-retention/config", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
...
function SystemDoctorPanel() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/admin/system/doctor", {
        cache: "no-store",
...
function ConfigMigrationPanel() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const [planResponse, ledgerResponse] = await Promise.all([fetch("/api/admin/config/migrations/plan?target=storage_retention_disk_watermark", {
        cache: "no-store",
        credentials: "same-origin"
      }), fetch("/api/admin/config/migrations", {
        cache: "no-store",
        credentials: "same-origin"
      })]);
  ...
  const apply = async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/admin/config/migrations/apply", {
        method: "POST",
        credentials: "same-origin",
        headers: {
          "Content-Type": "application/json"
...
function RuntimeEpisodeFabricPanel() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/admin/runtime-episodes/overview?limit=120", {
        cache: "no-store",
        credentials: "same-origin"
      });
      const data = await response.json().catch(() => null);
  ...
  const load = async () => {
    ...
    try {
      ...
      if (source === "all" || source === "runtime") {
        requests.push(fetch("/api/admin/runs?limit=40", {
          cache: "no-store"
        }).then(async response => ({
          kind: "runtime",
      ...
      if (source === "all" || source === "audit" || source === "hook" || source === "safety" || source === "storage") {
        ...
        requests.push(fetch(`/api/admin/audit/logs?${params.toString()}`, {
          cache: "no-store"
        }).then(async response => ({
          kind: "audit",
          payload: response.ok ? await response.json().catch(() => ({})) : {}
      ...
      if (source === "all" || source === "cron") {
        requests.push(fetch("/api/admin/cron/logs?limit=80", {
          cache: "no-store"
        }).then(async response => ({
          kind: "cron",
          payload: response.ok ? await response.json().catch(() => ({})) : {}
...
function EvidencePanel() {
  ...
  const load = async (mode: "replace" | "append" = "replace") => {
    ...
    try {
      ...
      const [observationsResponse, compactionsResponse] = await Promise.all([fetch(`/api/admin/observability/tool-observations?${query}`, {
        cache: "no-store"
      }), fetch("/api/admin/observability/compactions?limit=12", {
        cache: "no-store"
      })]);
  ...
  const reveal = async () => {
    ...
    setRevealing(true);
    try {
      const response = await fetch(`/api/admin/observability/tool-observations/${encodeURIComponent(selected.id)}/reveal`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
...
function RunLedgerPanel({
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/admin/runs/${encodeURIComponent(runId)}/ledger`, {
        cache: "no-store"
      });
      const payload = await response.json().catch(() => null);
      if (!response.ok) {
...
export default function OperationsCenterPage() {
  ...
  const {
    ...
    isRefreshing,
    refresh: loadSummary,
  } = useAdminJsonResource<SummaryPayload>("/api/admin/operations-center/summary", { ttlMs: 15_000 });
