...
export function AutomationDeliveryReview() {
    ...
    const load = useCallback(async (after?: string): Promise<LoadedPage | null> => {
        ...
        try {
            ...
            const response = await fetch(`/api/admin/automation/deliveries?${query}`, { signal: AbortSignal.any([request.signal, AbortSignal.timeout(15_000)]), cache: "no-store" });
            if (!response.ok) {
    ...
    const submit = async (event: FormEvent) => {
        ...
        const targetId = selected.deliveryId;
        try {
            const response = await fetch(`/api/admin/automation/deliveries/${encodeURIComponent(targetId)}/reconcile`, {
                method: "POST",
