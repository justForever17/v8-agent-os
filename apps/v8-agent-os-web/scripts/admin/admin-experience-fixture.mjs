...
export function adminExperienceFixture(url) {
    ...
    if (p === '/api/admin/ui-preferences/theme') return { theme: 'light' };
    if (p === '/api/admin/model-hub/bootstrap') return { providers: [sampleProvider], models: [sampleModel], hubEnvelope: { domain: 'model-hub', data: { models: [], providersOverview: [], summary: { providers: 1, models: 1 }, config: { governance: { enabled: true } } } }, defaultModel: { modelRef: sampleModel.modelRef }, catalog: { providers: [] }, audioConfig: {} };
    if (p === '/api/admin/providers/fixture-provider') return sampleProvider;
    if (p === '/api/admin/models') return [sampleModel];
    if (p === '/api/admin/agents') return [{ id: 'fixture-agent', name: 'Fixture specialist', description: 'Research and summarize', model_id: sampleModel.modelRef, tools: [], tool_mode: 'explicit', enabled: true, system_prompt: 'Fixture prompt', capabilitySnapshot: {}, createdBy: 'human' }];
    if (p === '/api/admin/agents/tool-surface') return { baselineSystemTools: [], runtimeManagedTools: [] };
    if (p === '/api/admin/extensions/catalog') return { skills: { items: [] }, mcp: { servers: [] }, summary: {} };
    if (p === '/api/admin/supervisor') return { name: 'Fixture supervisor', systemPrompt: 'Fixture prompt', model_id: sampleModel.modelRef, allowed_tools: [], locked_native_tools: [], runtime_managed_tools: [] };
    if (p === '/api/admin/mcp/tools') return { mcpTools: [] };
    if (p === '/api/admin/settings/vision-model') return { value: null, source: 'fixture' };
    if (p === '/api/admin/memory/dashboard') return { graph: { top_entities: [], entities: 36, relations: 33 }, preferences: [], statistics: {}, config: {} };
    if (p === '/api/admin/memory/knowledge' || p.includes('knowledge-resolution') || p.includes('/admin/memory/documents')) return { items: [] };
    if (p === '/api/admin/memory/knowledge-health') return { graph: { relations: 33 }, projection: {} };
    if (p === '/api/admin/memory/graph') {
    ...
    if (p.startsWith('/api/admin/config-registry/')) return { domain: p.split('/').at(-1), source: 'synthetic-ui-fixture', savePath: 'fixture', warnings: [], advancedFields: [], reloadRequired: false, data: {
