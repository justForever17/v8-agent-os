...
import { ModelSelect, type AdminModelSelectOption } from "@admin/components/models/ModelSelect";
...
const CREATIVE_BOOTSTRAP_PATH = "/api/admin/creative-media/bootstrap";
...
export default function CreativeMediaPage() {
    ...
    const fetchDiagnostics = useCallback(async (force = false) => {
        try {
            const governance = await fetchAdminJson<Record<string, unknown>>("/api/admin/creative-media/governance/snapshot", {
                force,
                ttlMs: 10_000,
            });
    ...
    const saveModelPreferences = useCallback(async () => {
        ...
        setError("");
        try {
            const response = await fetch("/api/admin/creative-media/model-preferences", {
                method: "POST",
    ...
    const mutateWorkOrder = useCallback(async (workOrderId: string, action: "archive" | "delete") => {
        ...
        setError("");
        try {
            const response = await fetch(`/api/admin/creative-media/governance/work-orders/${encodeURIComponent(workOrderId)}/${action}`, {
                method: "POST",
            });
            const payload = await response.json().catch(() => ({}));
            if (!response.ok) {
