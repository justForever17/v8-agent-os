...
const USER_MEDIA_FILENAME_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._-]{0,180}\.webp$/;
...
export function resolveUserMediaFile(kind: UserMediaKind, filename: string) {
  if (!USER_MEDIA_FILENAME_PATTERN.test(filename)) return null;
...
export function buildUserMediaPublicPath(kind: UserMediaKind, filename: string) {
  if (!USER_MEDIA_FILENAME_PATTERN.test(filename)) {
