...
function createHarness(cached, { realCache = false } = {}) {
  ...
  const root = path.resolve(__dirname, '../../src/admin');
  function load(name, parent = root) {
    if (name === 'react') return React;
    if (name === 'react/jsx-runtime') return { jsx, jsxs: jsx, Fragment: 'Fragment' };
    if (name === '@admin/lib/admin-client-cache' && !realCache) return {
    ...
    if ((name.startsWith('@admin/components/') && !name.startsWith('@admin/components/model-hub/')) || name === 'lucide-react' || name === 'next/image') return primitives;
    let filename = name.startsWith('@admin/') ? path.join(root, name.slice(7)) : path.resolve(parent, name);
  ...
  if (realCache && cached) load('@admin/lib/admin-client-cache').primeAdminJsonCache('/api/admin/model-hub/bootstrap', cached);
