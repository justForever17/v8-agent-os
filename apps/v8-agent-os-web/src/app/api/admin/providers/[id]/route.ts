...
import { buildStoredCredential, mapEngineProvider } from "@admin/lib/models/provider-admin";
import { canonicalizeOauthCredentialReference } from "@admin/lib/models/oauth-store";

const ENGINE_URL = resolveEngineBaseUrl();

export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    const session = await auth();
    if (!session?.user?.adminAuthenticated || session.user.role !== "ADMIN") return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    try {
