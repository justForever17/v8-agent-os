...
from .protocol import (CAPABILITIES, LEASE_MS, MAX_TTL_MS, MUTATING, TERMINAL,
...
class ExecutorService:
    ...
    def grant(self, owner: str, device: str, expected: int, allowed: list) -> dict:
        ...
        with self.identity.transaction() as db:
            ...
            permitted = {"device.health", "android.observe", "android.action"} if identity["device_class"] == "android" else {"device.health", "sensor.read", "actuator.set"}
    ...
    def hello(self, principal: dict, payload: dict) -> dict:
        ...
        allowed_class = {"device.health", "android.observe", "android.action"} if principal["device_class"] == "android" else {"device.health", "sensor.read", "actuator.set"}
    ...
    def create(self, *, owner: str, command_id: str, device: str, capability: str, resource: str,
        ...
        with self.identity.transaction() as db:
            ...
            if capability == "android.action":
                ...
                self._observation_current(db, device, hello, precondition)
            now = self.now()
    ...
    def _observation_current(self, db, device, hello, precondition):
        rows = db.execute("SELECT receipt,updated_at FROM executor_commands WHERE device_id=? AND state='succeeded' "
                          "AND receipt IS NOT NULL ORDER BY updated_at DESC LIMIT 16", (device,)).fetchall()
        for row in rows:
            ...
            if observation.get("observationId") == precondition.get("observationId"):
                ...
                require(all(observation.get(k) == precondition.get(k) for k in ("appId", "windowId", "nodeMapRevision")), "observation_target_stale")
                return
