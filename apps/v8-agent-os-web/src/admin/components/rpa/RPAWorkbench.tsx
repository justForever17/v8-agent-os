...
import { formatRunStatusLabel, formatWhen } from "@admin/components/runtime/use-runtime-ops";
import { AdminHoverInfo } from "@admin/components/admin-shell/AdminHoverInfo";
import { TechnicalReferenceDetails } from "@admin/components/common/TechnicalReferenceDetails";
import { ag, tg, ti } from "@admin/i18n/admin-legacy";
import { translateCurrentClient } from "@admin/lib/locale";
...
const RPA_AVAILABILITY_URL = "/api/admin/rpa/availability";
const RPA_SCRIPTS_URL = "/api/admin/rpa/scripts";
const RPA_APPROVALS_URL = "/api/admin/approvals?status=pending";
const RPA_RUNS_URL = "/api/admin/runs?limit=20";
const rpaDraftsUrl = (includeArchived: boolean) => `/api/admin/rpa/drafts?includeArchived=${includeArchived ? "true" : "false"}`;
const rpaTemplatesUrl = (includeArchived: boolean) => `/api/admin/rpa/templates?includeArchived=${includeArchived ? "true" : "false"}`;
...
export function RPAWorkbench() {
  ...
  const targetLockLooksLikeAdmin = useMemo(() => /v8 agent os|v8 os|localhost:9527|127\.0\.0\.1:9527|admin/i.test([
    canvasTargetAppId,
  ...
  const buildStepCaptureContext = (): StepCaptureContext | null => {
    ...
    const targetLock: Record<string, unknown> = {
      ...
      window: targetWindow,
      ignoreAdminSurface: true,
      consoleTargetBlocked: /v8 agent os|v8 os|localhost:9527|127\.0\.0\.1:9527|admin/i.test([
        appId,
        label,
        app?.topWindowTitle,
        targetWindow?.title,
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/conversations", { signal: controller.signal }).then(response => response.ok ? response.json() : [])
  ...
  const loadAll = useCallback(async (force = false) => {
    ...
    try {
      const [nextAvailability, nextDraftsPayload, nextScriptsPayload, nextTemplatesPayload, approvalsData, runsData, recordingsData] = await Promise.all([
        ...
        fetchAdminJson<{ recordings?: RecordingSessionPayload[] }>("/api/admin/rpa/recordings?limit=100", { force }),
      ]);
  ...
  const loadComputerApps = useCallback(async (force = false, queryOverride = "") => {
    ...
    try {
      ...
      const response = await fetch("/api/admin/computer-use/apps", {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        const res = await fetch(`/api/admin/rpa/templates/${encodeURIComponent(selectedTemplateId)}/history`, {
          cache: "no-store"
        });
  ...
  const refreshRecording = useCallback(async (recordingId: string) => {
    const res = await fetch(`/api/admin/rpa/recordings/${encodeURIComponent(recordingId)}`);
    const data = await res.json().catch(() => ({}));
  ...
  const handleCreateDraftFromStudio = async (options?: { saveAs?: boolean }) => {
    ...
    const data = await runAction(options?.saveAs ? "draft:create:save-as" : "draft:create", () => fetch("/api/admin/rpa/drafts", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const startComputerUseRecording = async () => {
    ...
    setRecordingError("");
    const data = await runAction("recording:computer-use:start", () => fetch("/api/admin/rpa/recordings/start", {
      method: "POST",
  ...
  const stopComputerUseRecording = async () => {
    ...
    setRecordingError("");
    const data = await runAction("recording:computer-use:stop", () => fetch(`/api/admin/rpa/recordings/${encodeURIComponent(activeRecording.recordingSessionId)}/stop`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const response = await fetch(`/api/admin/rpa/recordings/${encodeURIComponent(recordingId)}`, { cache: "no-store", signal: AbortSignal.any([controller.signal, AbortSignal.timeout(10_000)]) });
  ...
  const startRecordingSession = async (options?: { quiet?: boolean; captureContext?: StepCaptureContext | null }) => {
    ...
    const data = await runAction("recording:start", () => fetch("/api/admin/rpa/recordings/start", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const handleCaptureAssistantStart = async () => {
    ...
    try {
      ...
      const inspectorRes = await fetch(`/api/admin/rpa/recordings/${encodeURIComponent(recording.recordingSessionId)}/inspector/sessions`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  const handleOpenAgentBrowserAndRetryCapture = async () => {
    ...
    try {
      const res = await fetch("/api/admin/computer-use/agent-browser/open", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  const handleCaptureAssistantStop = async () => {
    if (!activeRecording?.recordingSessionId) return;
    const data = await runAction(`recording:native-inspector:stop:${activeRecording.recordingSessionId}`, () => fetch(`/api/admin/rpa/recordings/${encodeURIComponent(activeRecording.recordingSessionId)}/native-inspector/stop`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const handleCaptureAssistantPoll = useCallback(async (options?: { quiet?: boolean }) => {
    ...
    try {
      const res = await fetch(`/api/admin/rpa/recordings/${encodeURIComponent(activeRecording.recordingSessionId)}/native-inspector/poll`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        }
  ...
  const handleVerifyCapturePoolItem = async (item: CapturePoolItem) => {
    ...
    const res = await fetch(`/api/admin/rpa/recordings/${encodeURIComponent(activeRecording.recordingSessionId)}/capture-pool/${encodeURIComponent(item.tempElementId)}/verify`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const handleSaveCapturePoolItem = async (item: CapturePoolItem) => {
    ...
    const res = await fetch(`/api/admin/rpa/recordings/${encodeURIComponent(activeRecording.recordingSessionId)}/capture-pool/${encodeURIComponent(item.tempElementId)}/save`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const handleSampleComputerUse = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/rpa/recordings/${encodeURIComponent(recording.recordingSessionId)}/desktop-sample`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  const handleValidateSelectedStep = async (mode: "dry_run" | "selector" | "assertion") => {
    ...
    setBusyAction(actionKey);
    try {
      const res = await fetch(`/api/admin/rpa/drafts/${encodeURIComponent(selectedDraft.id)}/validate-step`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  const handlePatchDraftSteps = async () => {
    ...
    const data = await runAction(`draft:patch:${selectedDraft.id}`, () => fetch(`/api/admin/rpa/drafts/${encodeURIComponent(selectedDraft.id)}/patch`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const handleViewSourceTrace = async () => {
    ...
    await runAction(`draft:source:${selectedDraftId}`, () => fetch(`/api/admin/rpa/drafts/${encodeURIComponent(selectedDraftId)}/source-traces?include_steps=true&max_steps=8`, {
      cache: "no-store"
    }), tg(t, "846dc508"));
  };
  const handleDraftAction = async (mode: "export" | "prepare" | "run") => {
    ...
    await runAction(`draft:${mode}`, () => fetch(`/api/admin/rpa/drafts/${encodeURIComponent(savedDraft.id)}/${mode}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const handleDraftGovernanceAction = async (action: "archive" | "restore" | "delete") => {
    ...
    const endpoint = action === "delete" ? `/api/admin/rpa/drafts/${encodeURIComponent(selectedDraftId)}?confirm=true` : `/api/admin/rpa/drafts/${encodeURIComponent(selectedDraftId)}/${action}`;
  ...
  const handleApproveDraftAsTemplate = async () => {
    ...
    const data = await runAction(`draft:approve-template:${selectedDraft.id}`, () => fetch(`/api/admin/rpa/drafts/${encodeURIComponent(selectedDraft.id)}/approve-template`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const handleExistingAction = async (mode: "prepare-existing" | "run-existing") => {
    ...
    await runAction(mode, () => fetch(`/api/admin/rpa/${mode}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const handleApproval = async (approvalId: string, approve: boolean) => {
    ...
    const endpoint = approve ? `/api/admin/approvals/${approvalId}/approve` : `/api/admin/approvals/${approvalId}/reject`;
  ...
  const handleRunCommand = async (runId: string, command: "interrupt" | "retry") => {
    await runAction(`run:${command}:${runId}`, () => fetch(`/api/admin/runs/${runId}/commands/${command}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const handleTemplateAction = async (action: "approve" | "freeze" | "review_required") => {
    ...
    const endpoint = action === "approve" ? `/api/admin/rpa/templates/${encodeURIComponent(selectedTemplateId)}/approve` : action === "freeze" ? `/api/admin/rpa/templates/${encodeURIComponent(selectedTemplateId)}/freeze` : `/api/admin/rpa/templates/${encodeURIComponent(selectedTemplateId)}/review`;
  ...
  const handleTemplateGovernanceAction = async (action: "archive" | "restore" | "delete") => {
    ...
    const endpoint = action === "delete" ? `/api/admin/rpa/templates/${encodeURIComponent(selectedTemplateId)}?confirm=true` : `/api/admin/rpa/templates/${encodeURIComponent(selectedTemplateId)}/${action}`;
    await runAction(`template:${action}:${selectedTemplateId}`, () => fetch(endpoint, {
      method: action === "delete" ? "DELETE" : "POST",
      headers: action === "delete" ? undefined : {
        "Content-Type": "application/json"
  ...
  const handleTemplateRollback = async (revision?: number, historyPath?: string) => {
    ...
    await runAction(`template:rollback:${selectedTemplateId}:${revision || historyPath || "latest"}`, () => fetch(`/api/admin/rpa/templates/${encodeURIComponent(selectedTemplateId)}/rollback`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
