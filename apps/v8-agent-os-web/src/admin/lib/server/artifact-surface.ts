...
import { buildSignedClientSurfaceUrl } from "@admin/lib/server/client-surface-resource";
import {
    resolveClientSurfaceOriginFromRequest,
} from "@admin/lib/server/runtime-config";
...
function normalizeAdminSurfacePath(path: string) {
    ...
    if (normalized.startsWith("/api/admin/client/")) {
        return normalized;
    }
    if (normalized.startsWith("/api/client/")) {
        return normalized.replace(/^\/api\/client\b/i, "/api/admin/client");
    }
    if (normalized.startsWith("/api/memory/")) {
        return normalized.replace(/^\/api\/memory\b/i, "/api/admin/client/memory");
    }
...
export async function normalizeArtifactForAdminSurface(record: unknown, req: NextRequest) {
    ...
    const adminContentPath = `/api/admin/client/artifacts/${encodedId}/content?${adminContentParams.toString()}`;
