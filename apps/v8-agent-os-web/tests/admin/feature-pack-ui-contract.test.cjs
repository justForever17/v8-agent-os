...
test("Topbar uses feature pack API instead of legacy runtime install API", () => {
  const topbarSource = fs.readFileSync(path.join(adminRoot, "src", "admin", "components", "layout", "Topbar.tsx"), "utf8");
  const installerSource = fs.readFileSync(path.join(adminRoot, "src", "admin", "lib", "server", "runtime-feature-packs.ts"), "utf8");
  const featurePackSnapshotSource = fs.readFileSync(path.join(adminRoot, "src", "admin", "lib", "server", "engine-feature-pack-snapshot.ts"), "utf8");

  assert.match(topbarSource, /\/api\/admin\/runtime-feature-packs/);
  ...
  assert.match(topbarSource, /\/api\/admin\/runtime-feature-packs\?refresh=1/);
  ...
  const truthSource = fs.readFileSync(path.join(adminRoot, "src", "admin", "lib", "server", "runtime-feature-pack-truth.ts"), "utf8");
...
test("feature pack API exposes GET/POST and legacy runtime install is deprecated", () => {
  const routeSource = fs.readFileSync(path.join(adminRoot, "src", "app", "api", "admin", "runtime-feature-packs", "route.ts"), "utf8");
  const legacySource = fs.readFileSync(path.join(adminRoot, "src", "admin", "lib", "server", "runtime-install.ts"), "utf8");
  ...
  assert.match(legacySource, /replacement:\s*"\/api\/admin\/runtime-feature-packs"/);
...
test("feature pack cards localize metadata and do not expose raw installer errors", () => {
  const topbarSource = fs.readFileSync(path.join(adminRoot, "src", "admin", "components", "layout", "Topbar.tsx"), "utf8");
  const en = JSON.parse(fs.readFileSync(path.join(adminRoot, "src", "admin", "i18n", "locales", "en.json"), "utf8"));
  const zh = JSON.parse(fs.readFileSync(path.join(adminRoot, "src", "admin", "i18n", "locales", "zh-CN.json"), "utf8"));
...
test("desktop execution stays dependency-gated; RPA authoring remains available before runtime installation", () => {
  ...
  assert.ok(
    desktopSource.indexOf('if (enabled && featurePackState !== "ready")') < desktopSource.indexOf('fetch("/api/admin/runtime-capabilities/computer_use/policy"'),
  ...
  assert.match(rpaSource, /\/api\/admin\/rpa\/availability/);
...
test("feature packs publish immutable version targets with journal recovery and RPA dry-run smoke", () => {
  const installerSource = fs.readFileSync(path.join(adminRoot, "src", "admin", "lib", "server", "runtime-feature-packs.ts"), "utf8");
  const journalSource = fs.readFileSync(path.join(adminRoot, "src", "admin", "lib", "server", "feature-pack-transaction-journal.ts"), "utf8");
...
test("runtime navigation keeps beta only on dependency-gated runtimes and places them last", () => {
  const navSource = fs.readFileSync(path.join(adminRoot, "src", "admin", "lib", "admin-navigation.ts"), "utf8");
