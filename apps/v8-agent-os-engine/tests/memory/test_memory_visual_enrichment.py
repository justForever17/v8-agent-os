...
class _RunRecorder:
    ...
    def add_knowledge(self, **kwargs: Any) -> None:  # noqa: ANN401
...
def test_analyze_session_memory_feeds_visual_text_to_cheap_extractor(tmp_path: Path) -> None:
    ...
    with patch.object(memory_agent, "_load_memory_policy", return_value=_policy_with_defaults()), patch.object(memory_agent.db, "get_messages", return_value=[]), patch.object(memory_agent, "_build_canonical_session_transcript", return_value=transcript), patch.object(memory_agent.memory_runtime, "get_extraction_state", return_value=None), patch.object(memory_agent.storage, "get_memory_config", return_value={"visual_enrichment": {"enabled": True, "max_images": 2}}), patch.object(memory_agent, "_invoke_memory_visual_analyzer", return_value="--- Vision Analysis Complete ---\nOCR: 项目 test7，偏好深色界面。"), patch.object(memory_agent, "_generate_quick_summary", return_value="用户截图偏好"), patch.object(memory_agent, "_build_historical_context", return_value="No prior knowledge retrieved."), patch.object(memory_agent, "_extract_with_llm", side_effect=_fake_extract), patch.object(memory_agent.memory_runtime, "query_knowledge", return_value=[]), patch.object(memory_agent.memory_runtime, "save_extraction_state", side_effect=recorder.save_extraction_state), patch.object(memory_agent.memory_runtime, "upsert_preference", return_value=None), patch.object(memory_agent.memory_runtime, "add_knowledge", side_effect=recorder.add_knowledge), patch.object(memory_agent.memory_runtime, "append_daily_log_with_yaml", return_value=None), patch.object(memory_agent.knowledge_db, "add_entity", return_value=None), patch.object(memory_agent.knowledge_db, "add_relation", return_value=None), patch.object(memory_agent.audit_logger, "log", return_value=None), patch.object(memory_agent, "_run_incremental_index", return_value=None), patch.object(memory_agent.session_scope_binding_service, "get_binding", return_value=_binding(session_id)):
