...
Extract and install each upgrade in a **new** directory. While the old service is running, invoke `service upgrade --bundle /absolute/path/to/new/version` using the new package's CLI. Keep the old package available. The manager checks the database schema without importing Engine before starting a package. If readiness fails, it restores the prior unit and running state only when the previous Engine supports the current database. `service status --json` reports whether executable rollback is available and why it may be blocked.

Conversation revisions introduce database schema 4. A schema 3 Engine cannot open that migrated state, even if its package is still present. When an upgrade has migrated data and then fails readiness, the manager stops the failed candidate, preserves its unit, data and pending installation receipt, and refuses to start the incompatible old binary. After resolving the reported startup fault, use the new CLI's `service start` to retry that version, or `service upgrade --bundle /absolute/path/to/compatible/fixed/version` to recover forward. A manual incompatible rollback leaves a healthy newer service running.

Service rollback never restores a database snapshot or reverses data migrations. It does not overwrite newly created conversations, artifacts or credentials. Keep an independently verified state backup and its credential key before an upgrade requiring data migration; restoring such a backup is a separate, deliberate recovery operation. An old CLI does not gain these checks automatically, so perform upgrade and recovery with the new package's CLI.
