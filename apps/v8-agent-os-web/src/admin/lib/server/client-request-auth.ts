...
import { resolveMobileAccessUser } from "@admin/lib/mobile-auth";
import { verifyServiceAuth } from "@admin/lib/service-auth";
import { findUserByIdentifier, type AdminUserRecord } from "@admin/lib/users";
...
export async function resolveClientUser(req: NextRequest): Promise<AdminUserRecord | null> {
    ...
    if (!session?.user?.adminAuthenticated || session.user.role !== "ADMIN") {
        return null;
    }
