...
class ConfigDistributionService:
    ...
    @staticmethod
    def public(job):
        result = {key: deepcopy(job[key]) for key in ("jobId", "revision", "planDigest", "state", "templateId", "createdAt", "updatedAt")}
        result["targets"] = [{key: deepcopy(target[key]) for key in ("linkId", "peerId", "displayName", "state", "diff", "missingRequirements", "errorCode", "receipt") if key in target} for target in job["targets"]]
        return result
    ...
    def inventory(self, owner, issuer):
        ...
        return {"servingInstanceId": issuer, "templates": portable.templates(), "peers": peers,
                "jobs": [self.public(job) for job in self.store.jobs(owner)[:50]]}
    ...
    def create(self, owner, authority, body):
        ...
        for item in targets:
            ...
            portable.validate(template["id"], template["values"], mapping)
    ...
    def action(self, job_id, owner, action, body, *, authority=None):
        if set(body) - {"commandId", "revision", "planDigest"}:
            raise HTTPException(422, "distribution_fields_forbidden")
        command = identifier(body.get("commandId"))
        def change(job):
            ...
            if action == "confirm":
                ...
                for row in job["targets"]:
                    # An unknown prior apply must be reconciled, never replaced by a new plan.
                    if row["approved"] and row["state"] not in {"conflict", "blocked"}:
                        continue
                    self._assert_source_target(row)
                    row.update(generation=row["generation"] + 1, state="queued", approved=False, diff=[], receipt={}, errorCode="", missingRequirements=[])
                job.update(intent="prepare", state="preparing", planDigest="")
            elif action == "retry":
                if job["state"] in {"completed", "withdrawn", "cancelled"}:
    ...
    def _receive(self, peer_id, binding, action, payload):
        ...
        if record and generation > record["generation"] and (action != "prepare" or record.get("everCommitted") or record["state"] not in {"prepared", "blocked", "conflict"}):
        ...
        if action == "prepare":
            ...
            portable.validate(template_id, values, mapping)
            ...
            if action == "apply" and record["state"] in {"prepared", "applying"}:
                ...
                self.store.save_receipt(key, record)
            elif action == "status" and record["state"] in {"applying", "recovery_required"}:
                self._reconcile_received(record)
                self.store.save_receipt(key, record)
    ...
    async def _process_target(self, job_id, link_id):
        ...
        intent = job["intent"]
        if intent == "prepare" and (target["approved"] or target["state"] not in {"queued", "offline"}):
            return
        ...
        try:
            ...
            update.update(attempts=0, nextAttemptAt=0)
            ...
            for job in self.store.jobs():
                ...
                def settle(current):
                    ...
                    if current["intent"] == "prepare":
                        current["state"] = "awaiting_confirmation" if "prepared" in states else "partial"
                        ...
                        current["state"] = "withdrawn" if states <= {"rolled_back", "cancelled"} else "partial"
