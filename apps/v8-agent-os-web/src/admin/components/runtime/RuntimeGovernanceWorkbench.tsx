...
import { ApprovalRecord, RunRecord, formatRunStatusLabel, formatWhen } from "@admin/components/runtime/use-runtime-ops";
import { CORE_RUNTIME_KINDS, getRuntimeControlHref, getRuntimeDisplayText, isLockedRuntimeKind } from "@admin/lib/runtime-admin";
...
const RUNTIME_CAPABILITIES_URL = "/api/admin/runtime-capabilities";
const RUNS_URL = "/api/admin/runs?limit=40";
const APPROVALS_URL = "/api/admin/approvals?status=pending";
const CONVERSATIONS_URL = "/api/admin/conversations";
const MEMORY_DASHBOARD_URL = "/api/admin/memory/dashboard";
const MEMORY_AUDIT_URL = "/api/admin/audit/logs?source_type=MEMORY&limit=50";
...
export function RuntimeGovernanceWorkbench({
  ...
  const savePolicy = async (kind: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/runtime-capabilities/${encodeURIComponent(kind)}/policy`, {
        method: "POST",
        headers: {
  ...
  const applyPreset = async (preset: RuntimePresetId) => {
    ...
    try {
      await Promise.all(runtimes.map(async runtime => {
        ...
        const res = await fetch(`/api/admin/runtime-capabilities/${encodeURIComponent(runtime.kind)}/policy`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
