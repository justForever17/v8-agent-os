...
test("desktop pet settings use the authenticated config registry contract", () => {
  ...
  const routeSource = fs.readFileSync(
    path.join(adminRoot, "src", "app", "api", "admin", "config-registry", "[domain]", "route.ts"),
    "utf8",
  );
  const shellControlsSource = fs.readFileSync(
    path.join(adminRoot, "src", "admin", "components", "layout", "ShellWindowControls.tsx"),
    "utf8",
  );
  const zhLocale = fs.readFileSync(path.join(adminRoot, "src", "admin", "i18n", "locales", "zh-CN.json"), "utf8");
  const enLocale = fs.readFileSync(path.join(adminRoot, "src", "admin", "i18n", "locales", "en.json"), "utf8");

  assert.match(pageSource, /const API_PATH = "\/api\/admin\/config-registry\/desktop-pet"/);
