...
import { resolveAdminIdentity, proxyEngineJson } from "@admin/lib/server/engine-proxy";
...
async function proxy(req: NextRequest, context: Context) {
    ...
    if (setup && ((await auth())?.user?.adminAuthenticated !== true || (await auth())?.user?.role !== "ADMIN")) return NextResponse.json({ error: "Administrator required" }, { status: 403 });
    try {
