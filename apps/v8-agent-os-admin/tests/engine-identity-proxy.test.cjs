...
async function fixture(t) {
    ...
    const mocks = {
        ...
        '@/lib/server/runtime-config': { resolveEngineBaseUrl: () => origin + '/v1', resolveInternalSecret: () => 'synthetic-local' },
    };
    function load(name) {
        if (mocks[name]) return mocks[name];
        if (!name.startsWith('@/')) return require(name);
        if (cache.has(name)) return cache.get(name).exports;
        const file = path.resolve(__dirname, '../src', name.slice(2) + '.ts');
...
test('owner/profile reads and writes use Engine and preserve stable identity', async t => {
    const f = await fixture(t), users = f.load('@/lib/users');
...
test('refresh forwards the same rotation ID and a revoked bearer never gets service credentials', async t => {
    const f = await fixture(t), mobile = f.load('@/lib/mobile-auth');
...
test('service proof resolves Engine owner and ignores attacker supplied actor', async t => {
    const f = await fixture(t), service = f.load('@/lib/service-auth');
...
test('credential proxy rejects missing proof before Engine invocation', async t => {
    const f = await fixture(t), route = f.load('@/app/api/auth/verify-credentials/route');
...
test('a Phone bearer cannot use Admin BFF to create a management pairing ticket', async t => {
    const f = await fixture(t), route = f.load('@/app/api/client/pairing/tickets/route');
    const response = await route.POST(new NextRequest('http://admin.invalid/api/client/pairing/tickets', { method: 'POST', headers: { authorization: 'Bearer phone-token' }, body: '{}' }));
