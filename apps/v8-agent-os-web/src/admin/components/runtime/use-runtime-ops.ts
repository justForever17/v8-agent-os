...
import { getRuntimeDisplayText } from "@admin/lib/runtime-admin";
...
export function useRuntimeOpsData() {
        ...
        const request = (async () => {
            ...
            try {
                const [approvalRes, runRes] = await Promise.all([
                    fetch("/api/admin/approvals?status=pending", { cache: "no-store" }),
                    fetch("/api/admin/runs?limit=12", { cache: "no-store" }),
                ]);
    ...
    const dispatchRunCommand = useCallback(async (runId: string, command: "interrupt" | "retry") => {
        ...
        try {
            const response = await fetch(`/api/admin/runs/${runId}/commands/${command}`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
