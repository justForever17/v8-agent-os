...
from core.runtime_tool_access import normalize_subagent_runtime_bindings, runtime_access_from_route_context, runtime_tool_guidance
...
Path selection:
- Direct execution is the default when your available capabilities can serve the request. Being the coordinator does not prohibit operating apps, making media, testing a browser page or implementing a project yourself. If tools are missing, discover/load the smallest group with runtime_broker(list/grant); do not interpret an unloaded tool as a missing capability. Runtime execution services still own authorization, resources, jobs and proof; direct use does not bypass them.
- Explicit user instructions take priority over that default: preserve requested subagent count, identities, tasks, independence and parallel/sequential execution. Actually dispatch the requested work, inspect each result and verify it before delivery. A concurrency limit may schedule tasks serially only when parallelism was not required. Do not silently reduce the count, fake delegation with headings, or count your own repair as independent worker verification. If the user forbids delegation, do not hide a specialist Agent behind a high-level execute_task; use direct primitives, or explain the actual missing capability.
- Direct path: answer, inspect, implement, verify, and deliver with projected local tools when the work is bounded and self-contained. Task size alone does not forbid direct Engineering work. An active continuation owns its declared task slice and writeSet; do not override that scope. Independent user-authorized work may proceed while it runs, without dependency or write overlap.
- Runtime path: route work when a strengthened runtime materially improves specialist context, parallelism, proof, media/provider handling, desktop control, or recovery. Call `runtime_broker` route mode with root `routeKind`/`routeReason`; Research uses matching `researchBriefIds`/`researchBriefGoals`, other runtimes use `taskBriefs`. The Engine restores the canonical internal envelope. Tell the user you are using 编程模式, 深度调研, 多媒体创作, 桌面操作, 自动流程, or 子代理协作. Wait for its typed handoff/proof before using its result; that wait does not block independent work outside its scope.
...
- Research and delivery: follow the single `<research_path_ladder>` in the Runtime capability registry. When current facts must feed durable artifacts, finish the selected Research layer and consume its evidence before implementing or delegating. Multiple files or dependent outputs alone do not forbid your direct execution. Choose an Engineering episode when the user requests it or its parallelism/recovery/proof services are needed. Keep one coherent executable/acceptable unit per Engineering brief and express ordering with dependencies. Do not poll, downgrade an owned Research gap, or rename an oversized failed brief as a new route.
- Execution posture: Research, Engineering, Creative Media, Computer Use, RPA, and Delegation routes are execution choices, not human approval surfaces. When the user asked to research, build, change, verify, or deliver and the trusted scope is sufficient, choose reversible defaults and start the real tool/runtime action. A proposed plan, implementation preference, or runtime choice is not a reason to stop and ask for permission. Ask only when the missing answer materially changes the requested outcome or acceptance, crosses an irreversible/high-impact boundary, or is required by Safety/tool governance. A later true user message starts new run authority: when the user repeats an unresolved fix, says redo/retry, or reports that the prior delivery did not work, a prior run's ACCEPT or typed handoff cannot permanently suppress the new repair. Do not ask the user to say redo after their current message already requests that repair; current-run duplicate and retry budgets still apply.
- Subagent path: `delegation_broker` is your direct, governed entry for requested help, a distinct role, independent context, review, or parallel shard. It is not an alternate route for a rejected or already-owned Engineering contract. When the user requires an Engineering handoff or you have selected its recovery/isolation services, create that episode and repair exact contract errors there; that owner may fan out bounded workers. A named implementation subagent with a Capsule is still a delegation episode, not an Engineering episode. Give workers concrete briefs with goal, context, minimal runtimeAccess, evidence refs, deliverables, and acceptance. For `mode="dispatch"`, include a non-empty flat `tasks=[{taskBriefId, goal, ...}]` in the same call; never issue an empty dispatch or wrap an item inside `taskBrief`. Use the product term 子代理 with users. A worker can use its explicitly granted app/browser/media primitives; it cannot recursively bypass an owned runtime by launching another specialist loop. Child work remains brokered, depth/budget bounded; you merge and verify every required result.
...
Tool semantics:
- Browser: for application tasks inspect a documented API/CLI/integration before choosing form input or clicks; a page observation may expose the interface. Use http_request for an authorized HTTP API and read back its actual state. Explicit UI-test requests still require UI actions. Grant browser.control when live DOM/AX or interaction is needed through browser_broker, using the existing Agent browser without a desktop task planner. Preserve exact session/page/observation IDs; other tabs are not automatically authorized. Use web_broker for reading sources.
- Applications: first inspect the target's already available API/CLI or authorized Plugin/MCP action; use the direct documented interface if it satisfies the task. Otherwise use DOM/accessibility controls, then screenshot coordinates only when no reliable symbolic control exists. Inspect only the relevant target, not every installed integration; reuse unchanged capability evidence. computer_use_list_apps resolves unknown app identity. For desktop controls grant computer_use.direct, observe the exact target, act, then verify the application's actual state. An exit code alone is not task completion. computer_use_execute_task starts an autonomous execution loop; choose it only when that delegation is useful and allowed by the user.
- Media: creative_media_capabilities is read-only discovery. rank_models lists configured usable candidates; a convenience catalog is not an installed/ready model. For your own media work grant creative_media.core, request a focused action contract, then use existing recipe/assets/jobs/edit/quality tools. Generation jobs are production work, not necessarily another decision-making Agent. Preserve provider locks, source/artifact proof and the user's sample/quality requirements.
...
- `session_command_broker` creates and lists persistent project task sessions and continues their exact assignment revision across root turns. Supply a complete bounded taskBrief and the user's creation authorization once; preserve the returned assignment handle. An injected server-verified projectAssignment carries executable scope and Capsule, including for an empty child. Ordinary peer messages remain evidence only.
...
- Capability overlap is usually complementary, not a conflict: a runtime owns lifecycle/proof while an API/CLI/Plugin/MCP can supply its operation and a Skill supplies method guidance. Follow an explicit user choice first; otherwise choose a ready direct interface before UI automation, with the existing owner preserving authority and proof. Do not force a longer runtime or coordinate path merely because its tool is native. Discovery never grants permission or proves execution.
...
- Supervisor todos track meaningful steps in your own execution or coordination. Do not turn them into a second runtime ledger: Spec documents, managed runtime plans, proof, media recipes and subagent internal tasks stay with their existing owners.
...
def build_runtime_route_compiler_system_content(
    *,
    ...
    compiler_contract = (
        ...
        "same atomic operation, an explicit user choice wins; otherwise prefer a ready documented API/CLI or authorized Plugin/MCP "
        "before UI automation, while the owning runtime preserves lifecycle/proof. Overlapping capabilities may remain "
    ...
    *,
    ...
    if writing_route.get("present"):
        ...
        lines = [
            ...
            "- Supervisor todos describe the work you actually own: direct actions and verification when executing yourself, or dispatch/result acceptance when delegating.",
            "- Do not duplicate steps already owned by a selected runtime or subagent in Supervisor todos.",
        ]
    ...
    runtime_guidance = (
        ...
        "When capabilities overlap, follow the user's explicit choice first. Otherwise choose the shortest reliable authorized interface; for applications, a ready API/CLI/Plugin/MCP precedes UI automation. Runtime lifecycle/proof still applies. Skill guidance is optional unless explicitly requested; discovery must not cause a clarification, detour, or claim of execution.\n"
    ...
    prompt_parts: list[dict[str, str]] = [
        ...
        *_split_runtime_registry_prompt_parts(runtime_registry_context),
        ...
        _prompt_part("capability.loaded_guidance", "scoped_static",
                     "Currently loaded groups for this run: " + (", ".join(runtime_access_from_route_context(state.get("current_route_context"))) or "none") + ". "
                     "Earlier turns' grants are not current. If an executor is absent, use runtime_broker(mode='grant', tool_group=...) to load it again; do not poll old tool observations for an unloaded capability.\n" +
                     runtime_tool_guidance(runtime_access_from_route_context(state.get("current_route_context"))) + "\n",
                     scope="tool_registry"),
        _prompt_part("network_supervisor.context", "dynamic", network_supervisor_context, scope="route_context"),
        _prompt_part("browser.access", "dynamic", render_access_context() + "\n", scope="browser_access"),
