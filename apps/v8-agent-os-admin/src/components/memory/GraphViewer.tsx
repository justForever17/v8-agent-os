...
const ForceGraph2D = dynamic(() => import("react-force-graph-2d"), {
    ssr: false,
    loading: () => (
        <div className="flex h-[620px] items-center justify-center rounded-3xl border border-border/60 bg-muted/10">
...
export default function GraphViewer({ filterNode = "" }: GraphViewerProps) {
    ...
    const getNodeScale = useCallback((nodeId: string) => {
        ...
        if (isSelected) return 1.28;
        if (isHovered) return 1.14;
        if (isMatched) return 1.08;
        if (isSelectedNeighbor) return 1.02;
        return 0.94;
    ...
    const getLabelOpacity = useCallback((nodeId: string) => {
        ...
        if (isMatched) return 0.72;
        if (isRelated) return 0.3;
        if (hasSearchFocus) return 0;
        return 0.02;
    ...
    const loadGraph = useCallback(async () => {
        ...
        try {
            ...
            toast({
                title: t(lt("图谱加载失败", "Graph load failed")),
                description: t(lt("未能读取当前知识图谱。", "Unable to load the current graph.")),
                variant: "destructive",
            });
        } finally {
            setLoading(false);
    ...
    const loadEntityRelations = useCallback(async (entityId: string) => {
        try {
            ...
            setRelations([]);
            toast({
                title: t(lt("关系读取失败", "Failed to load relations")),
                description: t(lt("未能查询该节点的关系信息。", "Unable to query relations for this node.")),
                variant: "destructive",
            });
        }
    }, [t, toast]);
    ...
    const handleDeleteNode = useCallback(async () => {
        ...
        if (!window.confirm(t(lt(`确定删除节点 ${selectedNode.id} 及其相关关系吗？`, `Delete node ${selectedNode.id} and all related edges?`)))) {
            return;
        }
        ...
        try {
            ...
            toast({
                title: t(lt("节点已删除", "Node removed")),
                description: t(lt(`${selectedNode.label} 已从知识图谱移除。`, `${selectedNode.label} was removed from the graph.`)),
            });
            closeMenu();
            await loadGraph();
        } catch (error) {
            ...
            toast({
                ...
                description: t(lt("节点删除失败，请稍后重试。", "Failed to delete the node. Please try again.")),
                variant: "destructive",
            });
        } finally {
            setMutating(false);
    ...
    const handleCreateRelation = useCallback(async () => {
        ...
        if (!target || !predicate) {
            toast({
                title: t(lt("关系信息不完整", "Incomplete relation")),
                description: t(lt("目标节点和关系谓词都需要填写。", "Target node and predicate are required.")),
                variant: "destructive",
            });
            return;
        }
        ...
        try {
            ...
            toast({
                title: t(lt("关系已建立", "Relation created")),
            ...
            toast({
                title: t(lt("连接失败", "Create relation failed")),
                description: t(lt("关系建立失败，请稍后重试。", "Failed to create the relation. Please try again.")),
                variant: "destructive",
            });
        } finally {
            setMutating(false);
    ...
    const handleDeleteRelation = useCallback(async (relation: GraphRelation) => {
        ...
        try {
            ...
            toast({
                title: t(lt("关系已分离", "Relation removed")),
                description: t(lt(`${relation.subject} 与 ${relation.object} 的连接已移除。`, `Removed the link between ${relation.subject} and ${relation.object}.`)),
            });
            await refreshGraphState();
        } catch (error) {
            console.error("Failed to delete relation:", error);
            toast({
                title: t(lt("分离失败", "Detach failed")),
                description: t(lt("关系删除失败，请稍后重试。", "Failed to remove the relation. Please try again.")),
                variant: "destructive",
            });
        } finally {
            setMutating(false);
    ...
    if (!data || data.nodes.length === 0) {
        return (
            <div className="flex h-[620px] flex-col items-center justify-center rounded-3xl border border-dashed border-border/70 bg-muted/10">
                <p className="mb-2 text-muted-foreground">{t(lt("暂无知识图谱数据", "No graph data yet"))}</p>
                <p className="text-xs text-muted-foreground/70">{t(lt("随着您与 Agent 交互，系统会自动提取并建立知识图谱", "The graph is built automatically as you interact with the agent."))}</p>
            </div>
        );
    }
    ...
    return (
        <div
            ref={containerRef}
            className="relative h-[620px] w-full overflow-hidden rounded-3xl border border-border/60 bg-[radial-gradient(circle_at_center,rgba(99,102,241,0.12),transparent_42%),radial-gradient(circle_at_24%_22%,rgba(56,189,248,0.12),transparent_28%),radial-gradient(circle_at_78%_18%,rgba(16,185,129,0.08),transparent_24%),linear-gradient(180deg,rgba(15,23,42,0.16),rgba(2,6,23,0.02))]">
            ...
            <div className="absolute left-5 top-5 z-10 max-w-xs rounded-2xl border border-border/50 bg-background/70 px-4 py-3 backdrop-blur">
                <p className="text-xs uppercase tracking-[0.22em] text-muted-foreground/70">{t(lt("知识轨道", "Knowledge Orbit"))}</p>
                <p className="mt-2 text-sm text-muted-foreground">
                    {t(lt("节点会自然收敛在稳定布局中。悬停时标签渐显，点击节点会展开微菜单，便于直接连接、分离或删除。", "Nodes settle into a stable layout. Hover reveals labels, and clicking a node opens a micro menu to connect, detach, or remove it."))}
                </p>
            </div>
            ...
            <ForceGraph2D
                ...
                onEngineStop={() => fgRef.current?.zoomToFit?.(480, 110)}
                onNodeHover={(node) => setHoveredNodeId(node ? (node as GraphNode).id : null)}
                onNodeClick={(node, event) => void handleNodeClick(node, event as MouseEvent)}
                nodeLabel={(node) => {
                    const graphNode = node as GraphNode;
                    const degree = nodeDegrees.get(graphNode.id) || 0;
                    return locale === "en"
                        ? `${graphNode.label || graphNode.id} · ${graphNode.type} · ${degree} links`
                        : `${graphNode.label || graphNode.id} · ${graphNode.type} · ${degree} 条连接`;
                }}
                linkLabel={(link) => {
            ...
            {menuPosition && selectedNode ? (
                <div
                    ref={menuRef}
                    className="absolute z-20 w-[260px] rounded-3xl border border-border/60 bg-background/90 p-4 shadow-2xl shadow-black/20 backdrop-blur-xl animate-in fade-in-0 zoom-in-95"
                    ...
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">
                                {selectedNode.type} · {locale === "en" ? `${relations.length} relations` : `${relations.length} 条关联`}
                            </p>
                        </div>
                    ...
                    {menuMode === "connect" ? (
                        <div className="mt-4 space-y-3">
                            <div className="grid gap-2">
                                <label className="text-xs text-muted-foreground">{t(lt("目标节点", "Target node"))}</label>
                                <Input
                                    value={connectTarget}
                                    onChange={(event) => setConnectTarget(event.target.value)}
                                    placeholder={t(lt("输入目标节点名", "Enter a target node"))}
                                />
                            </div>
                            <div className="grid gap-2">
                                <label className="text-xs text-muted-foreground">{t(lt("关系谓词", "Predicate"))}</label>
                                <Input
                                    value={connectPredicate}
                                    onChange={(event) => setConnectPredicate(event.target.value.toUpperCase())}
                                    placeholder={t(lt("例如 USES / RELATES_TO", "e.g. USES / RELATES_TO"))}
                                    className="font-mono"
                                />
                            </div>
                            ...
                            <div className="rounded-2xl border border-border/50 bg-muted/20 p-3">
                                <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
                                    ...
                                    {t(lt("快速选择", "Quick pick"))}
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    {targetSuggestions.length === 0 ? (
                                        <span className="text-xs text-muted-foreground">{t(lt("没有匹配节点，也可以直接输入一个新名称。", "No matching nodes. You can also type a new node name."))}</span>
                                    ) : (
                                        targetSuggestions.map((node) => (
                                            <button
                                                key={node.id}
                                                type="button"
                                                className="rounded-full border border-border/60 px-2.5 py-1 text-xs transition hover:border-primary/40 hover:bg-primary/5"
                                                onClick={() => setConnectTarget(node.id)}
                                            >
                                                {node.label}
                                            </button>
                                        ))
                            ...
                            <div className="max-h-56 space-y-2 overflow-y-auto pr-1">
                                {relations.length === 0 ? (
                                    <div className="rounded-2xl border border-dashed px-3 py-4 text-xs text-muted-foreground">
                                        {t(lt("当前节点没有可分离的关系。", "This node has no detachable relations."))}
                                    </div>
                                ) : (
                                    relations.map((relation, index) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className="rounded-2xl border border-border/50 bg-muted/20 px-3 py-3"
                                            >
