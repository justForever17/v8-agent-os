...
function mount(cached) {
  ...
  const { useModelHubBootstrap } = harness.load('@/hooks/use-model-hub-bootstrap');
...
test('save clears only submitted audio draft and invalidates older reads', async () => {
  ...
  const { mergeAudioConfig } = ui.load('@/lib/model-hub/audio');
...
test('real cache keeps saved audio across remount and rejects a pre-save in-flight read', async () => {
  ...
  const useBootstrap = harness.load('@/hooks/use-model-hub-bootstrap').useModelHubBootstrap;
  ...
  const cache = harness.load('@/lib/admin-client-cache');
  assert.equal(cache.peekAdminJsonCache('/api/model-hub/bootstrap').audioConfig.tts.edge_tts.voice, 'saved voice');
...
for (const outcome of ['success', 'failure']) {
  test('unmount ignores late ' + outcome + ' without state writes', async () => {
    ...
    const ui = harness.mount(harness.load('@/hooks/use-model-hub-bootstrap').useModelHubBootstrap);
