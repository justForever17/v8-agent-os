...
import { searchAdminTopbarEntries } from "@admin/components/layout/admin-topbar-search";
import { LocaleToggle } from "@admin/components/layout/LocaleToggle";
import { ThemeToggle } from "@admin/components/layout/ThemeToggle";
import { DeviceConnectDialog } from "@admin/components/admin/DeviceConnectDialog";
import { useLocale, useT } from "@admin/components/providers/LocaleProvider";
import { useToast } from "@admin/components/ui/use-toast";
import { getAdminNavItem } from "@admin/lib/admin-navigation";
import { fetchAdminJson, primeAdminJsonCache } from "@admin/lib/admin-client-cache";
...
const WEB_CHAT_SURFACE_URL = "/api/admin/client/web-surface";
...
export function AdminTopbar({ windowControls }: { windowControls?: ReactNode }) {
    ...
    const loadInstallState = useCallback(async (force = false, silent = false, refreshHealth = false) => {
        ...
        try {
            ...
            if (refreshHealth) {
                primeAdminJsonCache("/api/admin/runtime-feature-packs", payload, 5_000);
            }
    ...
    const loadV8OSUpdateState = useCallback(async (force = false) => {
        ...
        try {
            ...
            if (force) {
                primeAdminJsonCache("/api/admin/v8os-update", payload, V8OS_UPDATE_CACHE_TTL_MS);
            }
    ...
    const loadInbox = useCallback(async (silent = false, force = false, refreshHealth = false) => {
        ...
        try {
            while (true) {
                try {
                    const url = requestRefreshHealth ? "/api/admin/admin-inbox?refresh=1" : "/api/admin/admin-inbox";
                    const payload = await fetchAdminJson<InboxPayload>(url, {
                        force: requestForce,
                        ttlMs: 10_000,
                    });
                    if (requestRefreshHealth) {
                        primeAdminJsonCache("/api/admin/admin-inbox", payload, 10_000);
                    }
