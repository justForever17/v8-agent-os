...
import { fetchConfigDomain, saveConfigDomain, type ConfigRegistryEnvelope } from "@admin/lib/config-registry";
import { cn } from "@admin/lib/utils";
import { ik, tg, ti } from "@admin/i18n/admin-legacy";
interface SysModel {
  id: string;
  modelRef?: string;
  providerId?: string;
...
export function MemoryContextPanel() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [contextEnvelope, modelList] = await Promise.all([fetchConfigDomain<ContextDomainData>("context"), fetch("/api/admin/models", {
        cache: "no-store"
