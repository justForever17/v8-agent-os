...
try {
    ...
    try {
        ...
        & $compiler @common /LD (Join-Path $PSScriptRoot 'provider.cpp') (Join-Path $PSScriptRoot 'protocol.cpp') (Join-Path $PSScriptRoot '../v8-native-common/windows.cpp') /Fe:V8SessionUnlock.dll /link /DYNAMICBASE /NXCOMPAT "/DEF:$(Join-Path $PSScriptRoot 'provider.def')" @libraries
        if ($LASTEXITCODE -ne 0) { throw 'Credential Provider build failed.' }
        & $compiler @common (Join-Path $PSScriptRoot 'client.cpp') (Join-Path $PSScriptRoot 'protocol.cpp') (Join-Path $PSScriptRoot '../v8-native-common/windows.cpp') /Fe:v8-session-unlock.exe /link /DYNAMICBASE /NXCOMPAT @libraries
        if ($LASTEXITCODE -ne 0) { throw 'Native client build failed.' }
        & $compiler @common (Join-Path $PSScriptRoot 'protocol_tests.cpp') (Join-Path $PSScriptRoot 'protocol.cpp') (Join-Path $PSScriptRoot '../v8-native-common/windows.cpp') /Fe:protocol-tests.exe /link /DYNAMICBASE /NXCOMPAT @libraries
        if ($LASTEXITCODE -ne 0) { throw 'Protocol tests build failed.' }
        & $compiler @common (Join-Path $PSScriptRoot 'provider_tests.cpp') (Join-Path $PSScriptRoot 'protocol.cpp') (Join-Path $PSScriptRoot '../v8-native-common/windows.cpp') /Fe:provider-tests.exe /link /DYNAMICBASE /NXCOMPAT @libraries
        if ($LASTEXITCODE -ne 0) { throw 'Provider tests build failed.' }
    } finally { Pop-Location }
    Write-Output "Built $Arch provider, client and non-installing protocol tests in $output"
} finally {
