...
def render_memory_extraction_prompt(format_instructions: str) -> str:
    return (
        ...
        "7. A memory item does NOT need to appear multiple times to be durable. A single explicit user-confirmed rule, architecture fact, runtime contract, environment convention, or standing protocol can still be durable.\n"
        "8. Repetition is supporting evidence, not a persistence requirement. Deduplicate repeats inside the same transcript. When a later transcript explicitly reaffirms an existing durable fact, emit one `reinforce` item with its exact `target_fact_id`; skip only casual repetition that adds no independent confirmation.\n"
        "9. Debugging conversations may still contain durable facts. Extract stable runtime contracts, project architecture rules, deployment conventions, or workspace agreements when the user explicitly confirms them.\n"
        "10. Extract `operational_workflow` knowledge when the conversation proves a reusable Computer Use, tool-use, browser, runtime, approval, media, or upload workflow that future agents should follow.\n"
        "11. Operational workflows may be global when they are product-level V8 Agent OS behavior, not a one-off project path or temporary command output.\n"
        "12. Local paths are durable only when they describe the current physical workspace, a deployment, or a download convention. Persist them to that workspace's exact resolved scope. One-off generated file paths should be `daily_log`, and path-like facts must never be promoted to `global`.\n"
        "13. If the conversation is mostly transient, keep `summary` and `tags`, but return empty lists for knowledge, preferences, entities, and relations.\n\n"
