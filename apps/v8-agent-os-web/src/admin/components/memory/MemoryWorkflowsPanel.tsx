...
export default function MemoryWorkflowsPanel() {
    ...
    const loadCandidates = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/admin/memory/workflows?${params.toString()}`, { cache: "no-store" });
    ...
    const loadSelected = useCallback(async (id: string) => {
        ...
        try {
            const [candidateRes, episodesRes, hintsRes] = await Promise.all([
                fetch(`/api/admin/memory/workflows/${encodeURIComponent(id)}`, { cache: "no-store" }),
                fetch(`/api/admin/memory/workflows/${encodeURIComponent(id)}/episodes`, { cache: "no-store" }),
                fetch(`/api/admin/memory/workflows/${encodeURIComponent(id)}/hint-events`, { cache: "no-store" }),
            ]);
    ...
    const saveSelected = useCallback(async (statusOverride?: string) => {
        ...
        try {
            ...
            const res = await fetch(`/api/admin/memory/workflows/${encodeURIComponent(selected.id)}`, {
                method: "PATCH",
    ...
    const deleteSelected = useCallback(async () => {
        ...
        const res = await fetch(`/api/admin/memory/workflows/${encodeURIComponent(id)}`, { method: "DELETE" });
        if (!res.ok) {
