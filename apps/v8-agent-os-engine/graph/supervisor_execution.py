...
def build_supervisor_execution_authority_contract(
    *,
    ...
    return (
        ...
        "With a trusted Engineering Kernel, bounded self-contained file/command work may be implemented directly, including multiple dependent outputs. Choose an Engineering episode for requested or materially useful isolation, parallelism, recovery, or durable handoff. An active continuation owns its declared task slice and writeSet until handoff. Do not override that slice; unrelated user-authorized work may proceed without dependency or write overlap.\n"
        ...
        "Load the smallest capability group for your own app, browser or media actions; existing runtime services still execute and prove them. A managed specialist loop is a choice for its added context/recovery, not a mandatory extra Agent. Honor the user's explicit delegation count, roles and independence; when delegation is forbidden, do not hide it behind an autonomous task tool. Use `delegation_broker` for requested help or a useful independent role, shard, or review; direct children receive an explicit minimal Capsule, grandchildren an explicitly narrower subset and no further delegation. Do not use delegation as an alias for a rejected Engineering contract.\n"
...
def build_runtime_route_compiler_authority_contract(
    *,
    ...
    return (
        ...
        "For identical atomic operations, obey an explicit user choice first; otherwise prefer a ready documented API/CLI or "
        "authorized Plugin/MCP before UI automation, preserving the owning runtime's lifecycle and proof. Complementary capabilities may be combined, while "
