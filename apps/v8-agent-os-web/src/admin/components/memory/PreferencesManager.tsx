...
export function PreferencesManager() {
  ...
  const loadPreferences = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/memory/preferences", { cache: "no-store" });
      if (!res.ok) {
  ...
  const handleRestoreQuarantined = useCallback(async (record: QuarantinedPreferenceRecord) => {
    try {
      const res = await fetch("/api/admin/memory/preferences/quarantine", {
        method: "POST",
  ...
  const saveRow = useCallback(async (scope: string, row: PreferenceRow) => {
    ...
    try {
      if (!row.isNew && row.originalKey && row.originalKey !== key) {
        await fetch("/api/admin/memory/preferences", {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ scope, key: row.originalKey })
        });
      }
      const res = await fetch("/api/admin/memory/preferences", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
