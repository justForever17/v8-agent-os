...
function baseline(relativePath) {
    ...
    return ref ? execFileSync("git", ["show", `${ref}:apps/v8-agent-os-web/${relativePath}`], { cwd: adminRoot, encoding: "utf8" }) : null;
}
function gateway(source) {
    ...
    const resource = load("src/admin/lib/server/session-realtime-resource.ts", { overrides: {
        "@admin/lib/server/client-surface-resource": { buildSignedClientSurfaceUrl: () => assert.fail("no asset in this fixture") },
        "@admin/lib/server/runtime-event-delivery": load("src/admin/lib/server/runtime-event-delivery.ts"),
    } });
    const gatewayModule = load("src/admin/lib/realtime/engine-chat-gateway.ts", { source, overrides: {
        "@admin/lib/realtime/session-fanout": { sessionFanoutHub: { publish: (id, event) => fanout.push({ id, event }) } },
        "@admin/lib/server/runtime-config": { resolveEngineWsBaseUrl: () => "ws://fixture.test", resolveInternalSecret: () => "public-test-key" },
        "@admin/lib/server/session-realtime-resource": resource,
    }, globals: {
        WebSocket: Socket,
...
test("unexpected EOF retains partial data and exact identity without publishing a failed run", async () => {
    ...
    const source = baseline("src/admin/lib/realtime/engine-chat-gateway.ts");
...
function commandRoutes(engine, { authorized = true, fetchImpl = fetch } = {}) {
    const overrides = {
        "@admin/lib/server/runtime-config": { resolveEngineBaseUrl: engine.origin, resolveEngineOrigin: () => engine.origin().replace(/\/v1$/, ""), resolveInternalSecret: () => "public-test-key" },
        "@admin/lib/server/request-auth": { resolveAuthorizedUserEmail: async () => authorized ? "fixture-owner" : null, unauthorizedJson: () => Response.json({ error: "Unauthorized" }, { status: 401 }) },
        "@admin/lib/auth": { auth: async () => authorized ? { user: { email: "fixture-owner" } } : null },
        "@admin/lib/service-auth": { verifyServiceAuth: async () => authorized ? "fixture-owner" : null },
    };
    const globals = {
        fetch: fetchImpl,
    ...
    overrides["@admin/lib/server/engine-fetch"] = load("src/admin/lib/server/engine-fetch.ts", {
        overrides: { "@admin/lib/server/runtime-config": overrides["@admin/lib/server/runtime-config"] },
        globals: { fetch: fetchImpl },
    });
    overrides["@admin/lib/server/engine-command-proxy"] = load("src/admin/lib/server/engine-command-proxy.ts", { overrides, globals });
    return ["approvals/[id]/approve", "approvals/[id]/reject", "runs/[runId]/commands/[command]"].map(value =>
        load(`src/app/api/admin/${value}/route.ts`, { overrides, globals }));
}
function request(signal) { return new NextRequest("http://fixture.test/api/admin/command", { method: "POST", body: '{"response":{"approved":true}}', signal }); }
...
test("invalid successful receipts cannot become 200 empty success; Engine conflict remains authoritative", async t => {
    ...
    const source = baseline("src/app/api/admin/approvals/[id]/approve/route.ts");
    if (!source) return;
    const old = load("src/app/api/admin/approvals/[id]/approve/route.ts", { source, overrides: {
        "@admin/lib/server/runtime-config": { resolveEngineBaseUrl: engine.origin, resolveInternalSecret: () => "public-test-key" },
        "@admin/lib/server/request-auth": { resolveAuthorizedUserEmail: async () => "fixture-owner" },
    } });
...
test("trusted chat sends minimal even without runtime mode; absent authorization remains absent", async () => {
    const payloads = [];
    const route = load("src/app/api/admin/chat/route.ts", { overrides: {
        "@admin/lib/volcengine": { generateImageWithDoubao: () => assert.fail("not an image request") },
        "@admin/lib/realtime/engine-chat-gateway": { createEngineChatGatewayStream: payload => { payloads.push(payload); return new ReadableStream({ start(c) { c.close(); } }); } },
        "@admin/lib/realtime/engine-chat-request": { validateSupervisorRuntimeMode: () => undefined },
        "@admin/lib/realtime/supervisor-runtime-mode": { SupervisorRuntimeModeValidationError: class extends Error {} },
        "@admin/lib/server/request-auth": { resolveAuthorizedUserEmail: async () => "fixture-owner" },
    } });
    for (const data of [{ safetyApprovalMode: "minimal" }, {}]) {
        const response = await route.POST(new NextRequest("http://fixture.test/api/admin/chat", { method: "POST", body: JSON.stringify({ messages: [{ role: "user", content: "task" }], data }) }));
...
test("Engine fetch boundary injects only canonical secret and preserves external stream headers", async () => {
    const calls = [];
    const loadedModule = load("src/admin/lib/server/engine-fetch.ts", {
        overrides: { "@admin/lib/server/runtime-config": {
