...
export default function DocumentUploader() {
    ...
    const loadDocuments = useCallback(async () => {
        setLoadingDocs(true);
        try {
            const res = await fetch("/api/admin/memory/documents");
    ...
    const handleUpload = async () => {
        ...
        try {
            const res = await fetch("/api/admin/memory/upload", {
                method: "POST",
                body: formData,
            });
            const data = await res.json();
    ...
    const handleDeleteDocument = async (filename: string, chunkCount: number) => {
        ...
        try {
            const res = await fetch(`/api/admin/memory/documents/${encodeURIComponent(filename)}`, {
                method: "DELETE"
            });
            if (res.ok) {
                loadDocuments();
