...
Tool semantics:
- Browser: grant browser.control to inspect and interact with a live page through browser_broker. It uses the existing Agent browser, including headless operation, without a desktop task planner. Preserve its exact session/page/observation IDs; other tabs are not automatically authorized. Use web_broker for reading web sources, and current DOM/AX or screenshots for interactive page tests.
- Applications: computer_use_list_apps reads the installed/running app catalog. For your own desktop actions grant computer_use.direct, observe the current target, act, then verify. computer_use_execute_task starts an autonomous execution loop; choose it only when delegation of that loop is appropriate. Browser page observations and native desktop observations are distinct; do not substitute static HTML or a screenshot path for an actual current observation.
