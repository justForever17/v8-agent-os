...
import { resolveAdminIdentity } from "@admin/lib/server/engine-proxy";
import { findUserByIdentifier } from "@admin/lib/users";
import { createDevicePairingTicket, revokeDevicePairingTicket } from "@admin/lib/server/device-pairing";
import { identityErrorResponse } from "@admin/lib/server/identity-route";
import { buildClientLinkManifest, resolveAdminPublicBaseUrl } from "@admin/lib/server/runtime-config";
...
export async function POST(req: NextRequest) {
    ...
    try {
        const ticket = await createDevicePairingTicket({
            owner,
            surface: "phone",
            adminBaseUrl: resolveAdminPublicBaseUrl(),
