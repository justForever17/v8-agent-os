...
export default function DesktopAutomationPage() {
    ...
    const [initialState] = useState(() => {
        ...
        return {
            ...
            availability: peekAdminJsonCache<ComputerUseAvailability>("/api/admin/computer-use/availability") ?? null,
    ...
    const loadData = async (force = false) => {
        ...
        try {
            const [config, modelsResponse, capabilitySnapshot, featurePacks] = await Promise.all([
                ...
                fetchAdminJson<Record<string, unknown>>("/api/admin/runtime-capabilities", { force }).catch(() => null),
                fetchAdminJson<{ packs?: RuntimeFeaturePackEntry[] }>("/api/admin/runtime-feature-packs", { force }).catch(() => null),
            ]);
            setEnvelope(config);
    ...
    useEffect(() => {
        ...
        const availabilityUrl = availabilityRefreshKey > 1
            ? "/api/admin/computer-use/availability?refresh=true"
            : "/api/admin/computer-use/availability";
        const loadAvailability = (attempt = 0) => {
            const requestUrl = attempt > 0 ? "/api/admin/computer-use/availability" : availabilityUrl;
    ...
    const setRuntimeEnabled = async (enabled: boolean) => {
        ...
        try {
            await fetch("/api/admin/runtime-capabilities/computer_use/policy", {
                method: "POST",
