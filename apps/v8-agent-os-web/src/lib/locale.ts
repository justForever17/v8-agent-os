...
function resolveMessage(locale: Locale, key: TranslationKey) {
    ...
    failOrWarn(`[web-i18n] Missing translation key "${key}" for locale "${locale}".`);
    return key;
}
...
function applyParams(key: TranslationKey, template: string, params?: TranslationParams) {
    return template.replace(PLACEHOLDER_PATTERN, (_, name: string) => {
        ...
        failOrWarn(`[web-i18n] Missing interpolation param "${name}" for key "${key}".`);
        return key;
    });
}
...
export function resolveText(locale: Locale, value: string, params?: TranslationParams) {
    if (!value) {
        return value;
    }
    ...
    if (/^[a-z0-9]+(?:[._-][a-z0-9]+)+$/i.test(value)) {
        failOrWarn(`[web-i18n] Unknown translation key "${value}" for locale "${locale}".`);
    }
    return value;
}
