...
import { CANONICAL_RUNTIME_KINDS, CORE_RUNTIME_KINDS, isCoreRuntimeKind, getRuntimeDisplayName, isCanonicalRuntimeKind, getRuntimeControlHref, getRuntimeDisplayText, isLockedRuntimeKind } from "@admin/lib/runtime-admin";
...
export function RuntimeDashboardCards() {
    ...
    const loadSnapshot = useCallback(async () => {
        setLoading(true);
        try {
            const response = await fetch("/api/admin/runtime-capabilities", { cache: "no-store" });
            if (!response.ok) {
    ...
    const handleToggle = useCallback(async (runtime: RuntimeDescriptor, checked: boolean) => {
        ...
        try {
            const response = await fetch(`/api/admin/runtime-capabilities/${encodeURIComponent(runtime.kind)}/policy`, {
                method: "POST",
