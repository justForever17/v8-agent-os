...
const ALLOWED_PATHS = [
    ...
    /^sessions\/[A-Za-z0-9_-]+\/canvas\/(?:actions|preview\/(?:source|artifact|workspace_asset)\/[A-Za-z0-9_.:-]+|psd\/(?:source|artifact|workspace_asset)\/[A-Za-z0-9_.:-]+\/(?:manifest|preview)|motion\/(?:source|artifact|workspace_asset)\/[A-Za-z0-9_.:-]+\/(?:manifest|frames\/\d+)|graph(?:\/(?:validate|history\/(?:undo|redo)|outputs\/[A-Za-z0-9_.:-]+\/(?:review|delivery)|runs(?:\/[A-Za-z0-9_.:-]+(?:\/(?:cancel|retry-failed-branch))?)?)?)?|templates(?:\/[A-Za-z0-9_.:-]+(?:\/instantiate)?)?)$/,
];
