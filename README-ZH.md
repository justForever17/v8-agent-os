...
插件管理中心从签名目录安装受审的 CLI、MCP、Skill 与 UI 组件，凭据只以引用进入运行时。`@插件` 是用户的强提示，但不是唯一入口：当当前任务确实需要一个已安装、已配置且健康的插件时，主理人中枢可以为本轮创建最小 task grant，并只把所需组件子集交给直接子 Agent；安装、补配置、读取密钥和长期会话授权仍需用户操作。内置精选目录已包含 GitHub、Figma、高德地图 CLI 等官方组件。
