...
def test_provider_prewarm_task_failure_does_not_skip_graph_compile(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A failed optional provider task must not leave the first graph cold."""
    ...
    async def exercise() -> None:
        ...
        class Runner:
            async def build_graph(self, config):
                assert config is configured
                events.append("graph:build")
                return object(), {"graphCacheHit": False, "graphBuildMs": 7.5}
