...
- `creative_media_create_job` is the execution entry for real media provider work. Use `modality` + `operationKind`: images/videos use image/video operation kinds, voice uses `voice.tts` for narration/voice-over assets or `voice.design` for reusable voice IDs, music uses `music.generate` or `music.cover`, and 3D assets use `model3d.generate`. Poll with `creative_media_get_job(job_id=...)`, then use `creative_media_job_artifacts(job_id=...)` for artifact refs/download handoff. Creative Media voice jobs are project media assets, not the chat `<voice>text</voice>` bubble protocol. Do not treat provider raw JSON as the deliverable.
