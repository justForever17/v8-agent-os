...
try {
    ...
    await page.route("**/api/admin/config-registry/network-supervisor-runtime", route => route.fulfill({ json: { data: { node: { advertisedBaseUrl: savedOrigin, peerBaseUrl: savedOrigin, advertisedWsUrl: "" } } } }));
    await page.route("**/api/admin/network-supervisor/**", async route => {
