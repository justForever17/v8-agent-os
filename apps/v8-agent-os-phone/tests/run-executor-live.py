...
def main():
    ...
    def api(path, body=None):
        ...
        request = urllib.request.Request("https://localhost:9533" + path, data=data, headers={"Content-Type": "application/json"})
    ...
    def terminal(command_id, timeout=12, require_device_receipt=True):
        ...
        while time.monotonic() < until:
            command = next((c for c in state()["commands"] if c["commandId"] == command_id), None)
    ...
    def wait_started(command_id):
        ...
        while time.monotonic() < until:
            result = next(c for c in state()["commands"] if c["commandId"] == command_id)
    ...
    def local(command):
        subprocess.run(["adb", "-s", args.serial, "shell", "am", "start", "-f", "0x10008000", "-n",
                        TEST_APP + "/expo.modules.v8executor.ExecutorTestActivity", "--es", "fixtureCommand", command],
    ...
    try:
        ...
        subprocess.run(["adb", "-s", args.serial, "shell", "am", "start", "-f", "0x10008000", "-n",
                        TEST_APP + "/expo.modules.v8executor.ExecutorTestActivity", "--es", "fixtureCommand", "stop"], check=True, capture_output=True)
        time.sleep(.5)
        ...
        time.sleep(2)
        local("status")
        probe = subprocess.run(["adb", "-s", args.serial, "logcat", "-d", "-s", "V8ExecutorFixture:I", "-v", "brief"],
                               check=True, capture_output=True, text=True, timeout=10).stdout
        assert "probe_enabled=false" in probe.splitlines()[-1], probe.splitlines()[-3:]
        ...
        subprocess.run(["adb", "-s", args.serial, "shell", "am", "start", "-f", "0x10008000", "-n",
                        TEST_APP + "/expo.modules.v8executor.ExecutorTestActivity", "--es", "fixtureCommand", "stop"],
