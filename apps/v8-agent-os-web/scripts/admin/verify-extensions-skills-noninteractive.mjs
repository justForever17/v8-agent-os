...
async function waitForServer() {
    ...
    while (Date.now() < deadline) {
        try {
            const response = await fetch(`${baseUrl}/api/admin/client/instance`);
...
try {
    ...
    await page.route("**/api/admin/extensions/health", (route) => jsonFulfill(route, {
        runtime: {
            startupState: "ready",
            snapshotFreshness: "live",
        },
    ...
    await page.route("**/api/admin/config-registry/extensions", (route) => jsonFulfill(route, extensionsConfigEnvelope()));
    await page.route("**/api/admin/models", (route) => jsonFulfill(route, []));
    await page.route("**/api/admin/skills/safety/reviews?**", (route) => jsonFulfill(route, { items: [] }));
    await page.route("**/api/admin/extensions/catalog", (route) => jsonFulfill(route, extensionsCatalog()));
    await page.route("**/api/admin/skills/install/command", async (route) => {
    ...
    const installResponsePromise = page.waitForResponse((response) => (
        response.url().endsWith("/api/admin/skills/install/command")
