...
class BaselineToolSurfaceTests(unittest.TestCase):
    def test_baseline_tool_descriptors_follow_canonical_common_package(self):
        ...
        self.assertNotIn("delegation_broker", names)
        self.assertIn("command_session_broker", names)
        self.assertIn("web_broker", names)
        self.assertNotIn("s3_broker", names)
        self.assertNotIn("share_workspace_file", names)
        self.assertIn("read_background_output", names)
        self.assertIn("send_background_input", names)
        self.assertIn("terminate_background_command", names)
