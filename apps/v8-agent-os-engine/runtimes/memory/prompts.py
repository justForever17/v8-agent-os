...
def render_memory_extraction_prompt(format_instructions: str) -> str:
    return (
        ...
        "3. Use the exact `resolved_scope` as the destination for workspace-specific preferences and facts; physical workspace identity is the ownership boundary.\n"
        "4. Use `global` for stable personal profile preferences that naturally apply to the user's conversations everywhere (language, tone, emoji/kaomoji style, formatting, persona, emotional boundaries), or when the transcript explicitly says a rule applies across all projects/workspaces. Keep project/tool/workflow preferences at the exact `resolved_scope`.\n"
        "5. Project IDs, channel IDs, display names, and transport thread IDs are metadata, not independent memory ownership scopes. Never invent or select a scope ID outside the runtime scope context.\n"
        ...
        "1. The Supervisor's self-name and the user's form of address are global identity truth, not workspace preferences.\n"
        "2. When the user explicitly names the Supervisor or changes how the Supervisor addresses them, emit those fields only in `identity`; never emit `assistant_name` or `user_call_name` in `preferences`.\n"
        "3. A naming instruction remains global even when it appears inside a workspace-specific task. Do not require the user to say 'global'.\n"
        "4. Do not infer, invent, or copy a name from role labels, agent cards, or a third party. Leave `identity` empty unless the user directly assigns it.\n\n"
        "SCORING RULES:\n"
        ...
        "8. Every emitted preference must include `evidence_role: \"user\"` and an exact `evidence_quote` copied from a USER message. Never use an assistant acknowledgement, inferred behavior, role label, or tool output as preference evidence.\n"
        "9. The preference value may normalize the user's wording but must not add a tone, style, or behavior that the evidence quote did not request.\n\n"
        "KNOWLEDGE RULES:\n"
        ...
        "2. Classify each fact with `relation`: `new`, `reinforce`, `replace`, `refine`, or `conflict`.\n"
        "3. Use `reinforce` only for the same meaning; use `replace` only when the user explicitly updates/corrects an old rule; use `refine` only for a confirmed additive fact that can coexist with the target; use `conflict` when the evidence is unresolved.\n"
        "4. An unverified alternative value for the same rule, field, schedule, path, owner, version, or configuration is always `conflict`, never `refine`, even when phrased as 'possibly', 'someone said', or 'not yet confirmed'.\n"
        "5. Use `new` only when no historical fact is being reaffirmed, changed, extended, or challenged. If the user explicitly says an item is a supplement/additional requirement and the old rule remains valid, use `refine` with that rule's exact ID even if the supplement could be written as an atomic sentence.\n"
        "6. For reinforce/replace/refine/conflict, set `target_fact_id` to the exact historical fact ID. Never infer replacement from text similarity alone. `overwrite_id` is deprecated and must be empty.\n"
        "7. If an `Explicit Correction Candidates` section is present, it is read-only evidence. Use a listed exact ID only when the user explicitly corrects, revokes, or replaces that old fact; stale entries must never be revived or updated by implication.\n"
        "8. A memory item does NOT need to appear multiple times to be durable. A single explicit user-confirmed rule, architecture fact, runtime contract, environment convention, or standing protocol can still be durable.\n"
        "9. Repetition is supporting evidence, not a persistence requirement. Deduplicate repeats inside the same transcript. When a later transcript explicitly reaffirms an existing durable fact, emit one `reinforce` item with its exact `target_fact_id`; skip only casual repetition that adds no independent confirmation.\n"
        "10. Debugging conversations may still contain durable facts. Extract stable runtime contracts, project architecture rules, deployment conventions, or workspace agreements when the user explicitly confirms them.\n"
        "11. Extract `operational_workflow` knowledge when the conversation proves a reusable Computer Use, tool-use, browser, runtime, approval, media, or upload workflow that future agents should follow.\n"
        "12. Operational workflows may be global when they are product-level V8 Agent OS behavior, not a one-off project path or temporary command output.\n"
        "13. Local paths are durable only when they describe the current physical workspace, a deployment, or a download convention. Persist them to that workspace's exact resolved scope. One-off generated file paths should be `daily_log`, and path-like facts must never be promoted to `global`.\n"
        "14. If the conversation is mostly transient, keep `summary` and `tags`, but return empty lists for knowledge, preferences, entities, and relations.\n\n"
        ...
        "5. Every relation must set `source_fact_index` to the zero-based knowledge item that proves it. If no emitted knowledge item proves the relation, omit the relation.\n\n"
        "IGNORE RULES:\n"
...
def render_memory_consolidation_prompt(format_instructions: str) -> str:
    return (
        ...
        "SCOPE SAFETY RULES:\n"
        "1. Respect physical workspace boundaries in the long-term memory plane.\n"
        "2. `global` is an independent shared read layer. Workspace memory belongs to exactly one physical workspace; project, channel, workflow, and display IDs are routing metadata, not parallel ownership scopes.\n"
        "3. Do not merge entities or relations across physical workspaces merely because their names are similar. Shared entity labels do not erase the scope of their evidence-backed relations.\n"
        "4. Do not recommend deleting facts across unrelated workspaces unless the contradiction is explicit and clear.\n"
...
def render_periodic_summary_prompt(
    *,
    ...
    return (
        ...
        "6. Source selection, changed ranges, and evidence refs were already computed by code. "
        "Do not discover sources or reinterpret scope; only synthesize the final narrative.\n"
        "7. If the material is thin, keep both fields short instead of inventing detail.\n\n"
        f"PERIOD: {tier}\n\n"
        f"RAW LOGS:\n{content}\n\n"
        f"FORMAT:\n{structured_output}\n"
    )
