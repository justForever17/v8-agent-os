...
function ModelViewerLoading() {
    const t = useT();
    return <div className="flex h-full items-center justify-center text-xs text-muted-foreground">{t("web.workbench.artifact.loading3d")}</div>;
}
...
function ArtifactMetadata({ artifact, document }: { artifact: RuntimeArtifact | null; document: ArtifactWorkbenchDocument }) {
    ...
    return (
        <div className="grid gap-x-6 gap-y-2 border-t border-border/60 px-3 py-2 text-[10px] text-muted-foreground sm:grid-cols-2">
            <div className="truncate"><span className="text-foreground/70">{t("web.workbench.artifact.type")}</span> · {artifact?.mimeType || artifact?.kind || document.renderer}</div>
            {artifact?.workspaceRelativePath ? <div className="truncate sm:col-span-2"><span className="text-foreground/70">{t("web.workbench.artifact.workspace")}</span> · {artifact.workspaceRelativePath}</div> : null}
        </div>
    );
}
    ...
    const preview = (() => {
        ...
        return (
            <div className="flex h-full flex-col items-center justify-center gap-3 px-8 text-center text-sm text-muted-foreground">
                ...
                <div className="font-medium text-foreground">{t("web.workbench.artifact.noPreview")}</div>
                <div>{t("web.workbench.artifact.noPreviewHint")}</div>
            </div>
        );
    })();
