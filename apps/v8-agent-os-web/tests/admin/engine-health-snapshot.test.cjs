...
const sourcePath = path.join(adminRoot, "src", "admin", "lib", "server", "engine-health-snapshot.ts");
...
function loadHealthModule(resolveOrigin = () => "http://engine.test") {
  ...
  const localRequire = (specifier) => specifier === "@admin/lib/server/runtime-config"
