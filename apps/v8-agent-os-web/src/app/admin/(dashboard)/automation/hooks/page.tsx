...
import { createTranslator } from "@admin/lib/locale";
...
function HooksPage() {
    ...
    const [hooks, setHooks] = useState<HookConfig[]>(() => peekAdminJsonCache<HooksConfigResponse>("/api/admin/hooks")?.hooks || []);
    const [isLoading, setIsLoading] = useState(() => !peekAdminJsonCache<HooksConfigResponse>("/api/admin/hooks"));
    ...
    const fetchHooks = useCallback(async (force = false) => {
        if (!peekAdminJsonCache<HooksConfigResponse>("/api/admin/hooks")) setIsLoading(true);
        try {
            const data = await fetchAdminJson<HooksConfigResponse>("/api/admin/hooks", { force });
            setHooks(data.hooks || []);
        }
        catch (e) {
    ...
    const saveHooksConfig = async (newHooks: HookConfig[]) => {
        try {
            ...
            if (res.ok) {
                setHooks(newHooks);
                primeAdminJsonCache("/api/admin/hooks", { hooks: newHooks });
    ...
    const handleToggle = async (name: string, currentEnabled: boolean) => {
        setIsToggling(name);
        try {
            const res = await fetch("/api/admin/hooks/toggle", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            if (res.ok) {
                setHooks((prev) => {
                    ...
                    primeAdminJsonCache("/api/admin/hooks", { hooks: next });
                    return next;
                });
            }
        }
