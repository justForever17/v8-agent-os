...
function mount(kind = 'admin', mutate = source => source, availability = { robotFramework: true }) {
  ...
  const files = {
    admin: '../../src/admin/components/rpa/RPAWorkbench.tsx',
    web: '../../src/components/rpa/RPAQuickPanel.tsx',
  };
  ...
  const imports = name => {
    ...
    if (name === './rpa-run-state') {
      const exports = {};
      new Function('exports', compile(fs.readFileSync(path.resolve(__dirname, '../../src/components/rpa/rpa-run-state.ts'), 'utf8')))(exports);
      return exports;
    }
