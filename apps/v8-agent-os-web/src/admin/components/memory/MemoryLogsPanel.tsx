...
export default function MemoryLogsPanel() {
    ...
    const loadFile = useCallback(async (relativePath: string) => {
        ...
        setLoadingFile(true);
        try {
            const response = await fetch(`/api/admin/memory/logs/file?path=${encodeURIComponent(relativePath)}`, { cache: "no-store" });
    ...
    const loadTree = useCallback(async (preferredPath?: string) => {
        setLoadingTree(true);
        try {
            const response = await fetch("/api/admin/memory/logs/tree", { cache: "no-store" });
            const payload = await response.json().catch(() => ({}));
            if (!response.ok) {
    ...
    const handleSave = useCallback(async () => {
        ...
        setSaving(true);
        try {
            const response = await fetch("/api/admin/memory/logs/file", {
                method: "PUT",
    ...
    const handleDelete = useCallback(async () => {
        ...
        setDeleting(true);
        try {
            const response = await fetch(`/api/admin/memory/logs/file?path=${encodeURIComponent(selectedPath)}`, { method: "DELETE" });
            const payload = await response.json().catch(() => ({}));
            if (!response.ok) {
