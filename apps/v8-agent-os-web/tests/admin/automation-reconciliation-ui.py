...
fs.writeFileSync(entry, `import React from 'react'; import {createRoot} from 'react-dom/client'; import {LocaleProvider} from '@admin/components/providers/LocaleProvider'; import {AutomationDeliveryReview} from '@admin/components/automation/AutomationDeliveryReview'; createRoot(document.getElementById('root')!).render(<LocaleProvider initialLocale={new URLSearchParams(location.search).get('locale') === 'en' ? 'en' : 'zh-CN'}><AutomationDeliveryReview/></LocaleProvider>);`);
webpack({mode:'production', context:admin, entry, output:{path:output,filename:'component.js'}, resolve:{extensions:['.tsx','.ts','.js','.json'],alias:{'@':path.join(admin,'src'),'@admin':path.join(admin,'src/admin')},modules:[path.join(admin,'node_modules'),'node_modules']}, module:{rules:[{test:/\.tsx?$/,use:loader}]}, optimization:{minimize:false}, performance:{hints:false}}, (error, stats) => {
...
def main() -> None:
    ...
    admin = Path(__file__).resolve().parents[2]
        ...
        with sync_playwright() as playwright:
            ...
            def mount(scenario: dict, *, locale="zh-CN", dark=False, width=1440, height=900):
                ...
                def route_request(route):
                    ...
                    if parsed.path == "/api/admin/automation/deliveries":
            ...
            for outcome in ("completed", "failed"):
                ...
                assert scenario["posts"][0] == {"url": "/api/admin/automation/deliveries/system-one/reconcile", "body": {"outcome": outcome, "evidence": {"observation": "public observation", "reference": "public-record-7"}}}
