...
import { ClipboardPaste, Copy, Link2, MousePointer2, Scissors } from "lucide-react";
...
export function AppContextMenu() {
    ...
    return createPortal(
        <div
            ref={menuRef}
            role="menu"
            aria-label={t("admin.contextMenu.label")}
            className={cn(
        ...
        >
            {hasEditableMenu ? (
                <>
                    {state.editable ? <MenuItem icon={Scissors} label={t("admin.contextMenu.cut")} disabled={!hasSelectedText} onSelect={async () => {
                        await writeClipboard(state.selectedText);
                        replaceSelection(state, "", "deleteByCut");
                        close();
                    }} /> : null}
                    <MenuItem icon={Copy} label={t("admin.contextMenu.copy")} disabled={!hasSelectedText} onSelect={async () => {
                        await writeClipboard(state.selectedText);
                        close();
                    }} />
                    {state.editable ? <MenuItem icon={ClipboardPaste} label={t("admin.contextMenu.paste")} onSelect={async () => {
                        try {
                    ...
                    <MenuItem icon={MousePointer2} label={t("admin.contextMenu.selectAll")} onSelect={() => {
                        selectAll(state);
                        close();
                    }} />
                </>
                ...
                <>
                    ...
                    {linkValue ? <MenuItem icon={Link2} label={t("admin.contextMenu.copyLink")} onSelect={async () => {
                        await writeClipboard(linkValue);
                        close();
                    }} /> : null}
                    <div className="-mx-1 my-1 h-px bg-border/70" />
                    <MenuItem icon={MousePointer2} label={t("admin.contextMenu.selectAll")} disabled={!state.selectionScope} onSelect={() => {
                        selectAll(state);
                        close();
                    }} />
                </>
