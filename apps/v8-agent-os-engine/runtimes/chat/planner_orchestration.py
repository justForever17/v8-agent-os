...
def planner_system_prompt() -> str:
    return (
        ...
        "- If a skill or user instruction asks for parallel agents, subagents, Agent Swarm, or many research shards, translate that into either a Research Runtime task for source-backed evidence or explicit top-level delegation workerBriefs. Do not assume a subagent can spawn child agents.\n"
        "- Child delegation is exceptional: only set allowChildDelegation with a small childDelegationBudget when a worker must coordinate a bounded nested subtask; otherwise split work at the top level or route broad research to Research Runtime.\n"
        ...
        "- For skill-generation research such as huashu-nuwa, the Phase 1 parallel research swarm should normally be a Research Runtime evidence task, or a top-level delegation dispatch with explicit workerBriefs and artifact paths; do not let the Supervisor serialize all web searches or expect grandchildren to appear implicitly.\n"
