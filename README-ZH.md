...
你可以把它理解为一个“主理人中枢”：你提出目标，它决定自己处理，还是交给编程模式、深度调研、多媒体创作、子代理协作等专门能力完成；你仍然能看到过程、接管关键决策，并在任务结束后拿到可验证的产物。
...
主理人中枢负责理解你的目标、选择合适路线，并在结果回流后做最后把关。它不是简单地把工具堆给模型，而是把工具、记忆、运行状态和产物组织成可追踪的流程。

### 专门工作模式

- 编程模式：处理项目创建、代码修改、测试和交付证明。
- 深度调研：做多源搜索、证据整理和调研包。
...
该命令会构建并启动本地桌面预览壳。完成后你会看到 V8OS 桌面窗口，而不是多个开发服务器页面。
...
V8OS 默认本地优先运行。桌面 Web、控制台和桌宠属于本机可信客户端；Phone 是远程客户端，需要配对连接。多设备协作、第三方应用接入和插件桥接属于高级能力，不会混入普通本机使用流程。

系统会尽量把普通用户界面保持清晰：你看到的是结果、风险、下一步和可打开的产物；内部调度、原始模型响应、审计记录和恢复信息保留在诊断面。
