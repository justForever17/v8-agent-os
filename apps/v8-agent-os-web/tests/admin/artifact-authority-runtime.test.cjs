...
function loadRequestAuth() {
  const serviceAuth = loadTypeScriptModule("src/admin/lib/service-auth.ts", {
    requireOverrides: {
      "@admin/i18n/internal-readable": { INTERNAL_READABLE: { k4b0c4c45f3: "missing secret" } },
      "@admin/lib/server/runtime-config": { resolveInternalSecret: () => internalSecret },
      "@admin/lib/server/engine-identity": { engineIdentity: async () => ({ user: { id: "fixture-owner", login: "owner", sessionIdentifier: "fixture-owner", email: "owner@example.invalid", role: "ADMIN" } }) },
    },
  });
  return loadTypeScriptModule("src/admin/lib/server/request-auth.ts", {
    requireOverrides: {
      "next/server": { NextRequest, NextResponse },
      "@admin/lib/auth": { auth: async () => null },
      "@admin/lib/service-auth": serviceAuth,
    },
  });
}
...
function memoryRouteOverrides(requestAuth, fetchImpl = global.fetch) {
  return {
    "next/server": { NextRequest, NextResponse },
    "@admin/lib/server/artifact-surface": {
      normalizeArtifactForAdminSurface: (value) => value,
      normalizeArtifactsForAdminSurface: (value) => value,
    },
    "@admin/lib/server/request-auth": requestAuth,
    "@admin/lib/server/runtime-config": { resolveEngineOrigin: () => "http://engine.test" },
    "@admin/lib/server/engine-fetch": { engineFetch: (input, init) => fetchImpl(input, init) },
  };
}
...
test("memory artifact list, detail, and content routes return 401 before Engine fetch", async () => {
  ...
  const listRoute = loadTypeScriptModule("src/app/api/admin/memory/artifacts/route.ts", options);
  const detailRoute = loadTypeScriptModule("src/app/api/admin/memory/artifacts/[id]/route.ts", options);
  const contentRoute = loadTypeScriptModule("src/app/api/admin/memory/artifacts/[id]/content/route.ts", options);

  const responses = await Promise.all([
    listRoute.GET(new NextRequest("http://admin.test/api/admin/memory/artifacts?sessionId=session-a")),
    detailRoute.GET(
      new NextRequest("http://admin.test/api/admin/memory/artifacts/artifact-a?sessionId=session-a"),
      { params: Promise.resolve({ id: "artifact-a" }) },
    ),
    contentRoute.GET(
      new NextRequest("http://admin.test/api/admin/memory/artifacts/artifact-a/content?sessionId=session-a"),
      { params: Promise.resolve({ id: "artifact-a" }) },
    ),
  ]);
...
test("valid service headers authorize memory content origin fetch with exact session scope", async () => {
  ...
  const contentRoute = loadTypeScriptModule("src/app/api/admin/memory/artifacts/[id]/content/route.ts", {
    requireOverrides: memoryRouteOverrides(requestAuth, fetchImpl),
    fetchImpl,
  });
  const req = new NextRequest(
    "http://admin.test/api/admin/memory/artifacts/artifact-a/content?sessionId=session-a&download=1",
...
test("signed content URL delegates the full session-scoped path to Engine", async () => {
  ...
  const runtimeConfig = {};
  const signing = loadTypeScriptModule("src/admin/lib/server/client-surface-resource.ts", {
    requireOverrides: { "@admin/lib/server/runtime-config": runtimeConfig, "@admin/lib/server/engine-identity": {
      engineIdentity: async (_path, init) => { internalRequest = { url: "engine-identity", init }; return { signedUrl: "/api/admin/client/artifacts/artifact-a/content?sessionId=session-a&v8exp=9&v8sig=engine" }; },
  ...
  const signedUrl = await signing.buildSignedClientSurfaceUrl(
    "/api/admin/client/artifacts/artifact-a/content?sessionId=session-a",
    { publicBaseUrl: "http://admin.test" },
  );

  assert.match(signedUrl, /v8sig=engine/);
  assert.deepEqual(JSON.parse(internalRequest.init.body), { path: "/api/admin/client/artifacts/artifact-a/content?sessionId=session-a", sessionId: "" });
