...
test("admin JSON cache preserves stale data while a refresh is in flight", () => {
    const source = read("src", "admin", "lib", "admin-client-cache.ts");
...
test("admin sidebar navigation does not fan out data reads from hover or focus", () => {
    const cacheSource = read("src", "admin", "lib", "admin-client-cache.ts");
    const sidebarSource = read("src", "admin", "components", "layout", "Sidebar.tsx");

    assert.match(cacheSource, /"\/admin\/model-hub": \[\["\/api\/admin\/model-hub\/bootstrap", 30_000\]\]/);
...
test("network supervisor prefetch uses the same plural neighbor and token routes as the page", () => {
    ...
    assert.match(source, /"\/api\/admin\/network-supervisor\/openai\/tokens"/);
    assert.match(source, /"\/api\/admin\/network-supervisor\/neighbors\/status"/);
    assert.match(source, /"\/api\/admin\/network-supervisor\/neighbors\/tasks\?limit=20"/);
...
test("operations prefetch avoids storage scans until the user opens retention details", () => {
    ...
    assert.match(operationsPrefetch, /"\/api\/admin\/operations-center\/summary"/);
...
test("operations center owns one runtime data hook and its summary route only reads health", () => {
    ...
    const approvalsSource = read("src", "admin", "components", "runtime", "PendingApprovalsPanel.tsx");
    const runsSource = read("src", "admin", "components", "runtime", "RecentRunsPanel.tsx");
    const summarySource = read("src", "app", "api", "admin", "operations-center", "summary", "route.ts");
...
test("model hub delegates cache hydration to its bootstrap hook", () => {
    const source = read("src", "admin", "hooks", "use-model-hub-bootstrap.ts");
...
test("observable resources refresh on focus without discarding their cached snapshot", () => {
    const source = read("src", "admin", "lib", "use-admin-json-resource.ts");
...
test("topbar defers inbox work and loads feature packs only on demand", () => {
    const source = read("src", "admin", "components", "layout", "Topbar.tsx");
    const inboxRoute = read("src", "app", "api", "admin", "admin-inbox", "route.ts");
    const healthCache = read("src", "admin", "lib", "server", "engine-health-snapshot.ts");
    ...
    assert.match(source, /primeAdminJsonCache\("\/api\/admin\/admin-inbox"/);
