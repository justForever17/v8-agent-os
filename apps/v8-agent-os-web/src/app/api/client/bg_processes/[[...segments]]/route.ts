...
async function relay(req: NextRequest, context: { params: Promise<{ segments?: string[] }> }) {
    ...
    const result = await safeAdminProxyFetch(auth.context, target, { method: req.method, headers: { "Content-Type": "application/json" }, ...(req.method === "POST" ? { body: await req.text() } : {}), signal: req.signal, cache: "no-store" }, "/client/bg_processes");
