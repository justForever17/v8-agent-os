...
当 Supervisor 在当前会话里调用工具创建定时任务时，任务会绑定到这个会话。后续定时触发产生的运行记录、消息和可恢复状态应回到同一个会话，不再在历史记录里额外切出一个“Cron 对话”。

这条规则只约束 Agent 工具创建的任务：

- 用户在会话中要求“每天提醒我 / 每周整理这个项目”时，后续输出应继续回到当前会话。
- Admin 页面手工创建的系统级定时任务仍可以是独立自动化任务。
- 如果任务需要发到外部渠道，应在 payload 中显式配置渠道，而不是伪装成普通聊天。
...
| 断点 | 期望结果 |
| --- | --- |
| 创建任务 | Agent 工具创建的任务带有当前 `session_id`，Admin 手工任务不强制绑定聊天 |
| 立即运行 | 执行入口能把 `session_id`、工作区和 payload 带入 Automation Runtime |
| 会话历史 | 附着任务不会在历史记录里单独生成 Cron 分组对话 |
| 并发触发 | 同一 job 正在运行时，下一次触发应被跳过或进入可观察状态，而不是并发写乱同一会话 |
| 中途失败 | run 状态应进入 failed / cancelled / review_required 等可诊断状态，并写入审计日志 |
| 后续恢复 | 可恢复任务应保留 run/session 绑定，避免恢复到新会话 |
