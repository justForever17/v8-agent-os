...
from core.chat_transcript_repository import ChatTranscriptRepository
...
class DatabaseManager:
    ...
    def __init__(self, db_path: Path):
        ...
        self._chat_transcript = ChatTranscriptRepository(self.get_connection, self._run_write_with_retry)
    ...
    def _init_db(self) -> None:
        with self.get_connection() as conn:
            initialize_database_schema(
                ...
                repair_known_schema_objects=self._repair_known_schema_objects,
            )
    ...
    # --- Canonical Chat Transcript Operations ---
    # Public DatabaseManager methods remain stable while the repository owns
    # transcript SQL, hydration, and revision/CAS semantics.

    def get_chat_transcript_state(self, session_id: str) -> Dict[str, Any]:
        return self._chat_transcript.get_chat_transcript_state(session_id)

    def assert_chat_run_epoch(self, session_id: str, run_id: str) -> None:
        return self._chat_transcript.assert_chat_run_epoch(session_id, run_id)

    def has_chat_branch_artifact_ref(self, session_id: str, artifact_id: str) -> bool:
        return self._chat_transcript.has_chat_branch_artifact_ref(session_id, artifact_id)

    def get_next_chat_canonical_ordinal(self, session_id: str) -> int:
        return self._chat_transcript.get_next_chat_canonical_ordinal(session_id)

    def create_chat_canonical_message(
        self, *, message_id: str, session_id: str, run_id: Optional[str], ordinal: int,
        role: str, state: str, nodes: list[dict[str, Any]],
        artifacts: Optional[list[dict[str, Any]]] = None, content_text: Optional[str] = None,
        reasoning_text: Optional[str] = None, metadata: Optional[dict[str, Any]] = None,
        finalized_at: Optional[str] = None,
    ) -> None:
        return self._chat_transcript.create_chat_canonical_message(
            message_id=message_id, session_id=session_id, run_id=run_id, ordinal=ordinal,
            role=role, state=state, nodes=nodes, artifacts=artifacts,
            content_text=content_text, reasoning_text=reasoning_text, metadata=metadata,
            finalized_at=finalized_at,
        )

    def update_chat_canonical_message(
        self, message_id: str, *, state: Optional[str] = None,
        nodes: Optional[list[dict[str, Any]]] = None, artifacts: Optional[list[dict[str, Any]]] = None,
        content_text: Optional[str] = None, reasoning_text: Optional[str] = None,
        metadata: Optional[dict[str, Any]] = None, finalized_at: Optional[str] = None,
        expected_version: Optional[int] = None,
    ) -> Optional[Dict[str, Any]]:
        return self._chat_transcript.update_chat_canonical_message(
            message_id, state=state, nodes=nodes, artifacts=artifacts,
            content_text=content_text, reasoning_text=reasoning_text, metadata=metadata,
            finalized_at=finalized_at, expected_version=expected_version,
        )

    def get_chat_canonical_message(self, message_id: str) -> Optional[Dict[str, Any]]:
        return self._chat_transcript.get_chat_canonical_message(message_id)

    def get_chat_canonical_message_by_run(
        self, *, session_id: str, run_id: str, role: str = "assistant"
    ) -> Optional[Dict[str, Any]]:
        return self._chat_transcript.get_chat_canonical_message_by_run(
            session_id=session_id, run_id=run_id, role=role
        )

    def get_chat_canonical_message_by_client_message_id(
        self, *, session_id: str, client_message_id: str, role: str = "user"
    ) -> Optional[Dict[str, Any]]:
        return self._chat_transcript.get_chat_canonical_message_by_client_message_id(
            session_id=session_id, client_message_id=client_message_id, role=role
        )

    def get_chat_canonical_messages(self, session_id: str) -> List[Dict[str, Any]]:
        return self._chat_transcript.get_chat_canonical_messages(session_id)

    def get_chat_canonical_messages_before_ordinal(
        self, session_id: str, before_ordinal: int | None = None, *, limit: int = 500
    ) -> List[Dict[str, Any]]:
        return self._chat_transcript.get_chat_canonical_messages_before_ordinal(
            session_id, before_ordinal, limit=limit
        )

    def get_chat_canonical_turn_index_rows(self, session_id: str) -> List[Dict[str, Any]]:
        return self._chat_transcript.get_chat_canonical_turn_index_rows(session_id)

    def get_chat_canonical_messages_in_ordinal_range(
        self, session_id: str, *, first_ordinal: int, last_ordinal: int
    ) -> List[Dict[str, Any]]:
        return self._chat_transcript.get_chat_canonical_messages_in_ordinal_range(
            session_id, first_ordinal=first_ordinal, last_ordinal=last_ordinal
        )

    def has_chat_canonical_message_before_ordinal(self, session_id: str, before_ordinal: int) -> bool:
        return self._chat_transcript.has_chat_canonical_message_before_ordinal(session_id, before_ordinal)

    def get_chat_canonical_messages_since(self, session_id: str, since_ts: str) -> List[Dict[str, Any]]:
        return self._chat_transcript.get_chat_canonical_messages_since(session_id, since_ts)

    def get_chat_message_deletions_since(self, session_id: str, since_ts: str) -> List[str]:
        return self._chat_transcript.get_chat_message_deletions_since(session_id, since_ts)

    def get_chat_canonical_max_version(self, session_id: str) -> int:
        return self._chat_transcript.get_chat_canonical_max_version(session_id)

    def _hydrate_chat_canonical_row(self, row: Dict[str, Any]) -> Dict[str, Any]:
        return self._chat_transcript._hydrate_chat_canonical_row(row)
