...
try {
    ...
    await ready(`${baseUrl}/api/admin/client/instance`, admin);
    ...
    const probeResponse = page.waitForResponse((response) => response.url().includes("/api/admin/config-registry/system-base?refresh=true"));
    releaseProbe();
    await probeResponse;
    await page.waitForTimeout(200);
    assert.equal(await addressInput.inputValue(), configured, "Late diagnostics must not overwrite typed Phone settings.");
    const saveResponsePromise = page.waitForResponse((response) => response.url().includes("/api/admin/config-registry/system-base") && response.request().method() === "POST");
    ...
    const savedManifest = await page.evaluate(async () => {
        const response = await fetch("/api/admin/client/link/manifest", { cache: "no-store" });
    ...
    const responsePromise = page.waitForResponse((response) => response.url().endsWith("/api/admin/client/pairing/tickets") && response.request().method() === "POST");
