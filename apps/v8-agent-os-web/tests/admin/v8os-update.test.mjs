...
import {
  ...
  readReleaseIdentity,
  selectV8OSUpdate,
} from "../../src/admin/lib/server/v8os-update.ts";
...
test("Topbar prefers the Shell update authority and keeps browser fallback read-only", () => {
  ...
  const topbar = fs.readFileSync(path.join(adminRoot, "src/admin/components/layout/Topbar.tsx"), "utf8");
  const shellApi = fs.readFileSync(path.join(adminRoot, "src/components/layout/ShellWindowControls.tsx"), "utf8");
  const route = fs.readFileSync(path.join(adminRoot, "src/app/api/admin/v8os-update/route.ts"), "utf8");
  const webSurfaceRoute = fs.readFileSync(
    path.join(adminRoot, "src/app/api/admin/client/web-surface/route.ts"),
    "utf8",
  );

  assert.match(shellApi, /getUpdateStatus\?: \(\) => Promise<(?:ShellUpdateStatus|any)>/);
  assert.match(shellApi, /checkForUpdates\?: \(\) => Promise<(?:ShellUpdateStatus|any)>/);
  ...
  assert.match(topbar, /WEB_CHAT_SURFACE_URL = "\/api\/admin\/client\/web-surface"/);
