...
import { ik, tg } from "@admin/i18n/admin-legacy";
...
export default function MemoryConfigPanel() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [confRes, modRes] = await Promise.all([fetch("/api/admin/settings/memory-config"), fetch("/api/admin/models")]);
      if (confRes.ok) {
  ...
  const handleRecallPreview = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/memory/recall-preview?${params.toString()}`);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await fetch("/api/admin/settings/memory-config", {
        method: "POST",
        headers: {
