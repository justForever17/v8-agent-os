...
function setup({ role = "ADMIN", authenticated = true, links = [], mutateStatus = 200 } = {}) {
  ...
  const mocks = {
    ...
    "@admin/lib/server/runtime-config": { buildClientLinkManifest: () => ({ instanceId: "authority-A" }), resolveRequestOrigin: () => "http://fixture.invalid" },
  };
  ...
  mocks["@admin/lib/server/client-supervisor-peers"] = load("src/admin/lib/server/client-supervisor-peers.ts");
  return { calls, load };
}
...
test("Phone peer list authenticates owner before any engine read", async () => {
  for (const options of [{ authenticated: false }, { role: "USER" }]) {
    ...
    const result = await fixture.load("src/app/api/admin/client/supervisor-peers/route.ts").GET(new NextRequest("http://fixture.invalid/api/admin/client/supervisor-peers"));
...
test("list contains only existing authorized links, minimal fields and explicit local session ownership", async () => {
  ...
  const result = await fixture.load("src/app/api/admin/client/supervisor-peers/route.ts").GET(new NextRequest("http://fixture.invalid/api/admin/client/supervisor-peers?limit=1"));
...
test("renaming binds the selected authorized link and cannot mutate endpoint or credential fields", async () => {
  ...
  const result = await fixture.load("src/app/api/admin/client/supervisor-peers/[linkId]/route.ts").PATCH(
    new NextRequest("http://fixture.invalid/api/admin/client/supervisor-peers/link-A", { method: "PATCH", body: JSON.stringify({ remoteNickname: "Office", endpoint: "https://untrusted.invalid", credentialRef: "no" }) }),
...
test("revoked / unknown links never reach timeline or mutation; errors cannot return success", async () => {
  const fixture = setup({ links: [link] });
  const unknown = await fixture.load("src/app/api/admin/client/supervisor-peers/[linkId]/timeline/route.ts").GET(new NextRequest("http://fixture.invalid/"), { params: Promise.resolve({ linkId: "other" }) });
  assert.equal(unknown.status, 404); assert.equal(fixture.calls.length, 1);
  const failed = setup({ links: [link], mutateStatus: 503 });
  const result = await failed.load("src/app/api/admin/client/supervisor-peers/[linkId]/route.ts").DELETE(new NextRequest("http://fixture.invalid/"), { params: Promise.resolve({ linkId: "link-A" }) });
  assert.equal(result.status, 503);
});
test("timeline preserves content but strips runtime envelopes and returns the actual serving location", async () => {
  const fixture = setup({ links: [link] });
  const result = await fixture.load("src/app/api/admin/client/supervisor-peers/[linkId]/timeline/route.ts").GET(new NextRequest("http://fixture.invalid/?before=42"), { params: Promise.resolve({ linkId: "link-A" }) });
