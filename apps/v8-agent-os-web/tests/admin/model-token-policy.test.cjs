...
function load(relative, suffix = "") {
    const source = fs.readFileSync(path.join(__dirname, "../../src/admin", relative), "utf8") + suffix;
    const compiled = ts.transpileModule(source, { compilerOptions: {
        module: ts.ModuleKind.CommonJS, jsx: ts.JsxEmit.ReactJSX,
    } }).outputText;
    const sandbox = { exports: {}, require: (name) => name === "@admin/lib/models/media-capabilities"
