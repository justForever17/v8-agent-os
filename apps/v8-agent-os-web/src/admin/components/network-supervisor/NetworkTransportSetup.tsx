...
export function NetworkTransportSetup({ node, onChange }: {
    ...
    React.useEffect(() => {
        ...
        fetch("/api/admin/network-supervisor/neighbors/setup", { signal: controller.signal, cache: "no-store" })
    ...
    const probe = async () => {
        ...
        try {
            const response = await fetch("/api/admin/network-supervisor/neighbors/setup/probe", {
    ...
    const save = async () => {
        ...
        try {
            const response = await fetch("/api/admin/network-supervisor/neighbors/setup", {
