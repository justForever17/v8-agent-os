...
const baseUrl = process.env.V8_ADMIN_TEST_URL || "http://127.0.0.1:9527";
...
try {
    ...
    const writes = [];
    await page.route("**/api/admin/model-hub/bootstrap*", (route) => route.fulfill({ json: {
        providers: [{ ...provider, models: [model] }], models: [model], hubEnvelope: control(), catalog: { providers: [] },
    } }));
    await page.route("**/api/admin/providers/fixture*", (route) => {
        assert.equal(route.request().method(), "GET");
        return route.fulfill({ json: { ...provider, models: [model] } });
    });
    await page.route("**/api/admin/model-control-plane*", (route) => route.fulfill({ json: control() }));
    ...
    for (const routeName of ["model-hub", "models/providers/fixture"]) {
        ...
        const save = async () => {
            const response = page.waitForResponse((item) => item.request().method() === "PUT" && item.url().includes("/api/admin/models/"));
