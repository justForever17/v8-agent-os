...
const sourcePath = path.join(adminRoot, "src", "admin", "lib", "realtime", "engine-chat-request.ts");
const runtimeModeSourcePath = path.join(adminRoot, "src", "admin", "lib", "realtime", "supervisor-runtime-mode.ts");
...
test("structured voice attachment wins over legacy fileUrls fallback", () => {
  const url = "/api/admin/client/resource/voice.mp3";
...
test("invalid Supervisor runtime mode is a structured 400 on trusted, paired, and durable ingress", async () => {
  ...
  const requireOverrides = {
    ...
    "@admin/lib/realtime/engine-chat-request": testModule.exports,
    "@admin/lib/realtime/supervisor-runtime-mode": runtimeModeModule.exports,
  ...
  const trustedStreamRoute = loadRouteModule("src/app/api/admin/chat/route.ts", requireOverrides);
  const streamRoute = loadRouteModule("src/app/api/admin/client/chat/route.ts", requireOverrides);
  const submitRoute = loadRouteModule("src/app/api/admin/client/chat-submit/route.ts", requireOverrides);
  const body = {
    data: {
      conversationId: "session-invalid-runtime-mode",
      supervisorRuntimeMode: "plugin_manager",
  ...
  const responses = await Promise.all([
    trustedStreamRoute.POST(request("/api/admin/chat")),
    streamRoute.POST(request("/api/admin/client/chat")),
    submitRoute.POST(request("/api/admin/client/chat-submit")),
  ]);
...
test("trusted stream forwards the normalized Supervisor runtime mode to Engine data", async () => {
  const { NextRequest, NextResponse } = require("next/server");
  let gatewayPayload;
  const route = loadRouteModule("src/app/api/admin/chat/route.ts", {
    "next/server": { NextRequest, NextResponse },
    "@admin/lib/realtime/engine-chat-request": testModule.exports,
    "@admin/lib/realtime/supervisor-runtime-mode": runtimeModeModule.exports,
    "@admin/lib/server/request-auth": {
  ...
  const response = await route.POST(new NextRequest("http://admin.test/api/admin/chat", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      data: {
...
test("paired stream and durable submit preserve a valid Supervisor runtime mode", async () => {
  ...
  const streamResponse = await streamRoute.POST(request("/api/admin/client/chat"));
  const submitResponse = await submitRoute.POST(request("/api/admin/client/chat-submit"));
...
test("explicit specialist mode cannot use the legacy Volcengine streaming shortcut", async () => {
  ...
  const trustedRoute = loadRouteModule("src/app/api/admin/chat/route.ts", requireOverrides);
  const clientRoute = loadRouteModule("src/app/api/admin/client/chat/route.ts", requireOverrides);
  const body = {
    data: {
      conversationId: "session-specialist-image",
      provider: "volcengine",
      fileUrls: ["/api/admin/client/resource/reference.png"],
  ...
  const responses = await Promise.all([
    trustedRoute.POST(request("/api/admin/chat")),
    clientRoute.POST(request("/api/admin/client/chat")),
  ]);
...
test("canonical Canvas dispatch metadata survives the Admin submit boundary", () => {
  const result = buildEngineChatRequestPayload({
    ...
    attachments: [{
      ...
      resourceRole: "source",
      url: "/api/admin/client/resource/source.png",
      mimeType: "image/png",
      mediaKind: "image",
    }],
    messages: [{
