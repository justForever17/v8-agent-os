...
export function readUsersPayload(): AdminUsersPayload {
    const data = readJson<{ users?: Record<string, unknown>[] }>("users.json", { users: [] });
    const rawUsers = Array.isArray(data.users) ? data.users : [];
    const normalized = rawUsers.map((item, index) => normalizeUserRecord(item, index));
    const payload = { users: sortUsers(normalized) };
...
export function writeUsersPayload(payload: AdminUsersPayload) {
    writeJson("users.json", { users: sortUsers(payload.users) });
}
...
export function updateUserRecord(
    id: string,
    patch: Partial<Pick<AdminUserRecord, "login" | "name" | "role" | "password" | "image" | "mustChangePassword" | "email">>,
) {
    const payload = readUsersPayload();
