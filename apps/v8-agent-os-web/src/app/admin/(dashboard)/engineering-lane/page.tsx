...
import { fetchAdminJson } from "@admin/lib/admin-client-cache";
import {
  fetchConfigDomain,
  peekConfigDomain,
  saveConfigDomain,
...
export default function EngineeringLanePage() {
  ...
  const loadProof = async (force = false) => {
    ...
    try {
      ...
      const data = await fetchAdminJson<{ items?: ProofEntry[] }>(`/api/admin/engineering-lane/proof-ledger?${params.toString()}`, { force, ttlMs: 10_000 });
  ...
  const loadWorksetObservations = async (force = false) => {
    ...
    const data = await fetchAdminJson<{ items?: WorksetObservationEntry[] }>(`/api/admin/engineering-lane/workset-observations?${params.toString()}`, { force, ttlMs: 10_000 });
  ...
  const loadEngineeringWorkflowCandidates = async (force = false) => {
    ...
    const data = await fetchAdminJson<{ items?: EngineeringWorkflowCandidate[] }>(`/api/admin/memory/workflows?${params.toString()}`, { force, ttlMs: 10_000 });
  ...
  const refreshSelectedProof = async () => {
    ...
    try {
      await fetch("/api/admin/engineering-lane/proof-ledger/refresh", {
        method: "POST",
