...
def render_memory_extraction_prompt(format_instructions: str) -> str:
    return (
        ...
        "4. Local paths are durable only when they describe a scoped project/channel workspace, deployment, or download convention. One-off generated file paths should be `daily_log`, and global path-like facts should not be durable.\n"
        "5. If the conversation is mostly transient, keep `summary` and `tags`, but return empty lists for knowledge, preferences, entities, and relations.\n\n"
        ...
        "6. When an item is useful only for recent continuity, downgrade it to `daily_log` instead of `knowledge`.\n\n"
        "GRAPH RULES:\n"
        "1. Keep entity names lowercase and concise.\n"
        "2. Extract only durable entities and relations.\n"
        "3. Do not extract transient Skills/Tools troubleshooting as graph knowledge; only extract durable runtime contracts, project rules, or stable integration facts.\n"
        ...
        "2. If the conversation is only about how to use the Skills/Tools system itself, do not extract it as memory or graph data. If it establishes a durable project rule or runtime contract, extract the contract, not the transient troubleshooting trace.\n\n"
        "OUTPUT QUALITY:\n"
