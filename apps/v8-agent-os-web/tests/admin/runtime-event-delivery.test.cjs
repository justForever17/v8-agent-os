...
const sourcePath = path.join(__dirname, "..", "..", "src", "admin", "lib", "server", "runtime-event-delivery.ts");
...
const streamRouteSource = fs.readFileSync(
  path.join(__dirname, "..", "..", "src", "app", "api", "admin", "realtime", "sessions", "[id]", "stream", "route.ts"),
  "utf8",
);
